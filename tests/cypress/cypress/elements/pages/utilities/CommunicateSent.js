...
class CommunicateSent extends ControlPanel {
  ...
  send(Subject,Body,Group){
    cy.get('input[name="subject"]').type(Subject)
    cy.get('textarea[name="message"]').type(Body)
    cy.get('input[type="checkbox"]').first().click()
    cy.get('input[value="Send Email"]').first().click()
  }

  send_detail(Subject,Body,Group,cc,bcc){
    cy.get('input[name="subject"]').type(Subject)
    cy.get('textarea[name="message"]').type(Body)
    cy.get('input[type="checkbox"]').first().click()
    cy.get('input[name="cc"]').type(cc)
    cy.get('input[name="bcc"]').type(bcc)
  ...
  load() {
    ...
    this.get('heading').contains('Sent Emails')
    this.get('phrase_search').should('exist')
    this.get('search_submit_button').should('exist')
    this.get('email_table').should('exist')
  }
}
