...
context('Display field short names on Publish', {browser: 'electron'}, () => {
  ...
  const available_fields = [
    ...
    it('Field short names shown by default', () => {
      ...
      cy.window().its('navigator.permissions').invoke('query', { name: 'clipboard-read' }).its('state').then(cy.log) // do this first to avoid error
      ...
      cy.window().its('navigator.clipboard').invoke('readText').then(text => {
        expect(text).to.eq('{stupid_grid:text_one}')
      })
      ...
      cy.window().its('navigator.clipboard').invoke('readText').then(text => {
        expect(text).to.eq('{stupid_grid:text_two}')
      })
      ...
      cy.window().its('navigator.clipboard').invoke('readText').then(text => {
        expect(text).to.eq('{corpse:rel_item}')
      })
      ...
      cy.window().its('navigator.clipboard').invoke('readText').then(text => {
        expect(text).to.eq('{corpse:stupid_grid}')
      })
