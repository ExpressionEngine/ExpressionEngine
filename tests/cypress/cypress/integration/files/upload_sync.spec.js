...
context('Upload Sync', () => {
  ...
  it('should sync the directory', () => {
    ...
    cy.task('filesystem:list', {target: images_path}).then((dir_files) => {
  ...
  it('should not overwrite existing manipulations if not told to', () => {
    ...
    let file = ''
    cy.task('filesystem:list', {target: upload_path + '_some_name', mask: '/*.png'}).then((files) => { file = files[0]})
    let stats = {}
    cy.task('filesystem:info', file).then((stat) => { stats = stat})
  ...
  it('should not sync invalid mimes', () => {
    cy.task('filesystem:copy', { from: bad_files+'*', to: upload_path })
