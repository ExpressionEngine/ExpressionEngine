...
context('Upload Destination Create/Edit', () => {
  ...
  it('should validate image manipulation data', () => {
    ...
    page.hasErrors()
    page.get('error_messages').should('have.length', 4)
  ...
  it('should repopulate the form on validation error, and save', () => {
    ...
    // page.submit()
    // cy.hasNoErrors() This wont work it gives an error prior to being able to submit, rather: check that values are still saved then do
    ...
    page.hasNoErrors()
    page.submit()
    cy.hasNoErrors()//move what we had down to here
