...
"");b.css("backgroundImage","none");h=i('<img src="'+e+'"/>');h.css({border:0,display:"none"}).width(l);i("body").append(h);b.data("img",h)}var e=c.start.top||Math.round(f.height()/2),m=c.start.left||Math.round(f.width()/2);g&&(g=k(g),e=g.top,m=g.left);h.css({position:"absolute",top:e,left:m,width:0,zIndex:c.zIndex}).show();a.top+=f.scrollTop();a.left+=f.scrollLeft();a.position="absolute";b.css(a);h.animate({top:b.css("top"),left:b.css("left"),width:l},c.speed,function(){c.fixed&&(a.top-=f.scrollTop(),
