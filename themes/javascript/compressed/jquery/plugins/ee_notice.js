...
if(h.last==a){var d=d.children().slice(-1),g=d.find(".subcount");g.length?g.text(parseInt(g.text())+1):d.prepend('<span class="subcount">2</span>')}else d.append("<p>"+a+"</p>"),h.last=a}else if(e.type=="custom")b(".notice_texts.notice_custom").html(a);else throw"Invalid notification type.";(e.type=="error"||e.open)&&m(e.type);b("#notice_flag").css("background","#67bb25");b("#notice_flag").stop().animate({backgroundColor:"#1F2B33"},1E3);return b.ee_notice}};b.ee_notice.destroy=function(){c&&o()};
b.ee_notice.show_info=function(a){c||k();b("#notice_flag").css("display","inline");b(".notice_info").show();p=a;g()};b.ee_notice.hide_info=function(){b(".notice_info").hide();var a=0;b.each(f,function(b,c){a+=c.count});a||b("#notice_flag").hide()}})(jQuery);
