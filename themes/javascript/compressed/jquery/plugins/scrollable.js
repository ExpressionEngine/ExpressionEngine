...
/*
 ...
 Date:    Tue Apr 20 19:26:58 2010 +0000 
*/
(function(d){function i(e,c){var a=d(c);return 2>a.length?a:e.parent().find(c)}function s(e,c){var a=this,k=e.add(a),f=e.children(),l=0,m=c.vertical;j||(j=a);1<f.length&&(f=d(c.items,e));d.extend(a,{getConf:function(){return c},getIndex:function(){return l},getSize:function(){return a.getItems().size()},getNaviButtons:function(){return n.add(p)},getRoot:function(){return e},getItemWrap:function(){return f},getItems:function(){return f.children(c.item).not("."+c.clonedClass)},move:function(b,c){return a.seekTo(l+
...
var i=d.Event("onBeforeSeek");if(!e&&(k.trigger(i,[b,g]),i.isDefaultPrevented()||!h.length))return a;h=m?{top:-h.position().top}:{left:-h.position().left};f.animate(h,g,c.easing,e||function(){k.trigger("onSeek",[b])});j=a;l=b;return a}});d.each(["onBeforeSeek","onSeek","onAddItem"],function(b,g){d.isFunction(c[g])&&d(a).bind(g,c[g]);a[g]=function(b){d(a).bind(g,b);return a}});if(c.circular){var q=a.getItems().slice(-1).clone().prependTo(f),r=a.getItems().eq(1).clone().appendTo(f);q.add(r).addClass(c.clonedClass);
a.onBeforeSeek(function(b,c,d){if(!b.isDefaultPrevented()){if(-1==c)return a.seekTo(q,d,function(){a.end(0)}),b.preventDefault();c==a.getSize()&&a.seekTo(r,d,function(){a.begin(0)})}});a.seekTo(0,0)}var n=i(e,c.prev).click(function(){a.prev()}),p=i(e,c.next).click(function(){a.next()});if(!c.circular&&1<a.getSize())a.onBeforeSeek(function(b,d){n.toggleClass(c.disabledClass,0>=d);p.toggleClass(c.disabledClass,d>=a.getSize()-1)});c.mousewheel&&d.fn.mousewheel&&e.mousewheel(function(b,d){if(c.mousewheel)return a.move(0>
