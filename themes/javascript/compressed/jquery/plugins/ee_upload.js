...
a=b.onStart(e,b.data)||{};d.extend(f,a);g.attr("action",n());l()};n=function(){return b.url+"&frame_id="+c.attr("id")+"&"+jQuery.param(f)};o=function(){c.remove();g.remove();h()};i=function(a){if(!d(c).data("upload_complete")&&"about:blank"!==(a.contentDocument?a.contentDocument:a.contentWindow?a.contentWindow.document:window.frames[c.attr("id")].document).location.href){if(window.EE_uploads[c.attr("id")])b.onComplete(window.EE_uploads[c.attr("id")],e,b.data);else b.onFailure("Connection timed out. Please try again.",
