...
var c,k,d,e,l;b.mask={load:function(a,g){if(d)return this;"string"==typeof a&&(a={color:a});a=a||e;e=a=b.extend(b.extend({},n.conf),a);c=b("#"+a.maskId);c.length||(c=b("<div/>").attr("id",a.maskId),b("body").append(c));var f=m();c.css({position:"absolute",top:0,left:0,width:f[0],height:f[1],display:"none",opacity:a.startOpacity,zIndex:a.zIndex});f=c.css("backgroundColor");(!f||"transparent"==f||"rgba(0, 0, 0, 0)"==f)&&c.css("backgroundColor",a.color);if(!1===h(a.onBeforeLoad))return this;a.closeOnEsc&&
b(document).bind("keydown.mask",function(a){27==a.keyCode&&b.mask.close(a)});a.closeOnClick&&c.bind("click.mask",function(a){b.mask.close(a)});b(window).bind("resize.mask",function(){b.mask.fit()});g&&g.length&&(l=g.eq(0).css("zIndex"),b.each(g,function(){var a=b(this);/relative|absolute|fixed/i.test(a.css("position"))||a.css("position","relative")}),k=g.css({zIndex:Math.max(a.zIndex+1,"auto"==l?0:l)}));c.css({display:"block"}).fadeTo(a.loadSpeed,a.opacity,function(){b.mask.fit();h(a.onLoad)});d=
