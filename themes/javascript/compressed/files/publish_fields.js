...
(function(b){function j(a){var b=!1;return a?(a=a.toString(),a=a.replace(/\(\!\(([\s\S]*?)\)\!\)/g,function(a,b){var c=b.split("|!|");return!0===altKey?void 0!==c[1]?c[1]:c[0]:void 0===c[1]?"":c[0]}),a=a.replace(/\[\!\[([\s\S]*?)\]\!\]/g,function(a,g){var c=g.split(":!:");if(!0===b)return!1;value=prompt(c[0],c[1]?c[1]:"");null===value&&(b=!0);return value})):""}function k(a,d){var e=b("input[name='"+d+"']").closest(".file_field");!1==a.is_image?e.find(".file_set").show().find(".filename").html('<img src="'+
...
a.find(".no_file"),f=b(this).data("content-type"),h=b(this).data("directory"),i=[],f={content_type:f,directory:h};b.ee_filebrowser.add_trigger(d,b(this).attr("name"),f,k);fileselector=d.length?d:c;a.find(".remove_file").click(function(){a.find("input[type=hidden]").val(function(a,b){i[a]=b;return""});a.find(".file_set").hide();a.find(".sub_filename a").show();fileselector.show();return!1});a.find(".undo_remove").click(function(){a.find("input[type=hidden]").val(function(a){return i.length?i[a]:""});
...
(d.markItUp(myNobuttonSettings),d.closest(".markItUpContainer").find(".markItUpHeader").hide(),d.focus()),a.is_image?b.markItUp({replaceWith:e}):b.markItUp({key:"L",name:"Link",openWith:c,closeWith:f,placeHolder:a.file_name})):d.val(function(a,b){b+=c+e+f;return j(b)})})};EE.publish.file_browser.file_field=function(){h("input[type=file]","#publishForm, .pageContents");Grid.bind("file","display",function(a){h("input[type=file]",a)})};EE.publish.file_browser.category_edit_modal=function(){h("input[type=file]",
"#cat_modal_container")};b(function(){!0==EE.filebrowser.publish&&setTimeout(function(){EE.publish.file_browser.file_field();EE.publish.file_browser.textarea()},15)})})(jQuery);
