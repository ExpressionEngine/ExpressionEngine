...
EE.file_manager.sync=function(a){if(EE.file_manager.sync_files.length<=0)return EE.file_manager.finish_sync();var c=EE.file_manager.sync_files.splice(0,5);$.ajax({url:EE.BASE+"&C=content_files&M=do_sync_files",type:"POST",dataType:"json",data:{XID:EE.XID,upload_directory_id:a,sizes:EE.file_manager.sync_sizes,files:c},beforeSend:function(){console.log("start");EE.file_manager.sync_running+=1},complete:function(){console.log("finish");EE.file_manager.sync_running-=1;EE.file_manager.sync(a);var b=EE.file_manager.sync_file_count;
