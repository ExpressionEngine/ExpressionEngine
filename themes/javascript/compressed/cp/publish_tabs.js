...
drop:function(a,b){field_id=b.draggable.attr("id")=="hide_title"||b.draggable.attr("id")=="hide_url_title"?b.draggable.attr("id").substring(5):b.draggable.attr("id").substring(11);tab_id=$(this).attr("id");$("#hold_field_"+field_id).prependTo("#"+tab_id);$("#hold_field_"+field_id).hide().slideDown()}});$(".tab_menu li.content_tab a, #publish_tab_list a.menu_focus").unbind(".publish_tabs").bind("mousedown.publish_tabs",function(a){tab_id=$(this).attr("title").substring(5);tab_focus(tab_id);a.preventDefault()}).bind("click.publish_tabs",
