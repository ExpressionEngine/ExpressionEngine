...
function setup_tabs(){var a="";$(".main_tab").sortable({connectWith:".main_tab",appendTo:"#holder",helper:"clone",forceHelperSize:!0,handle:".handle",start:function(a,b){b.item.css("width",$(this).parent().css("width"))},stop:function(a,b){b.item.css("width","100%")}});$(".tab_menu li a").droppable({accept:".field_selector, .publish_field",tolerance:"pointer",forceHelperSize:!0,deactivate:function(){clearTimeout(a);$(".tab_menu li").removeClass("highlight_tab")},drop:function(a,b){field_id=b.draggable.attr("id").substring(11);
...
drop:function(a,b){field_id="hide_title"==b.draggable.attr("id")||"hide_url_title"==b.draggable.attr("id")?b.draggable.attr("id").substring(5):b.draggable.attr("id").substring(11);tab_id=$(this).attr("id");$("#hold_field_"+field_id).prependTo("#"+tab_id);$("#hold_field_"+field_id).hide().slideDown()}});$(".tab_menu li.content_tab a, #publish_tab_list a.menu_focus").unbind(".publish_tabs").bind("mousedown.publish_tabs",function(a){tab_id=$(this).attr("title").substring(5);tab_focus(tab_id);a.preventDefault()}).bind("click.publish_tabs",
function(){return!1})}$(".tab_menu li:first").addClass("current");setup_tabs();
