...
$.ee_filemanager.directory_change=function(){var b=EE.file.directoryInfo,a=RegExp("!-!","g");$.each(b,function(d,e){$.each(e,function(c,e){var f=new String;$.each(e,function(b,c){f+='<option value="'+c[0]+'">'+c[1].replace(a,String.fromCharCode(160))+"</option>"});b[d][c]=f})});$("#dir_id").change(function(){var a=this.value;void 0===b[a]&&(a=0);jQuery.each(b[a],function(a,b){$("select#cat_id").empty().append(b)})})};
$.ee_filemanager.date_range=function(){function b(){"yyyy-mm-dd"!=$("#custom_date_start").val()&&"yyyy-mm-dd"!=$("#custom_date_end").val()&&(focus_number=$("#date_range").children().length,$("#date_range").append('<option id="custom_date_option">'+$("#custom_date_start").val()+" to "+$("#custom_date_end").val()+"</option>"),document.getElementById("date_range").options[focus_number].selected=!0,$("#custom_date_picker").slideUp("fast"),$("#date_range").change(),oTable.fnDraw())}$("#custom_date_start_span").datepicker({dateFormat:"yy-mm-dd",
