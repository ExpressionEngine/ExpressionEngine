...
a._checkScrollBars()})},_bindSortable:function(){var a=this,b,c;c=function(b){return+a.defaultList[a._index(b)].find("input:text").val()};this.active.find("ul").sortable({axis:"y",handle:".reorder-handle",start:function(a,e){b=c(e.item)},update:function(c,e){var f=e.item,d=f.index()+1,h;d!=b&&(d<b?(h=d,f=f.nextAll().andSelf()):(h=1,f=f.prevAll().andSelf()),f.each(function(){a.defaultList[a._index(this)].find("input:text").val(h++)}))}})},_index:function(a){return d(a).closest("li").data("list-index")},
...
var d=document.createElement("ul");_.each(c,function(a){d.appendChild(b[a][0])});var e=_.toArray(d.childNodes),f=e.length,g=0;(function j(){a.append(e.slice(g++,100));g<f&&_.defer(j)})()},_disallowClickSelection:function(){var a=0,b=this;this.root.dblclick(b._deselect).click(function(){a++;_.debounce(function(){a=0},500);2<=a&&b._deselect()})},_deselect:function(){window.getSelection?window.getSelection().removeAllRanges():document.selection&&document.selection.empty()}};EE.setup_relationship_field=
function(a){return new i(a)}})(jQuery);
