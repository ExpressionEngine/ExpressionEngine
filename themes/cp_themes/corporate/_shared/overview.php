		...
		<div class="overview">
			...
			<table class="mainTable" cellspacing="0" cellpadding="0" border="0">
					...
					<?php if ($this->input->get('C') == 'content' && $menu_key == 'publish'):?>
						...
						<td class="overviewItemHelp"><a rel="external" href="<?=$this->config->item('base_url').$this->config->item('index_page').'?URL='.$this->menu->generate_help_link($controller, $menu_key, $controller)?>"><img src="<?=$cp_theme_url?>images/external_link.png"/></a></td>
					<?php else:?>
				...
				<table class="mainTable" cellspacing="0" cellpadding="0" border="0">
					...
					?>
						...
						<?php if ($this->input->get('C') == 'content' && $menu_key == 'publish'):?>
							...
							<td class="overviewItemDesc">&nbsp;</td>
							<td class="overviewItemHelp"><a rel="external" href="<?=$this->config->item('base_url').$this->config->item('index_page').'?URL='.$this->menu->generate_help_link($controller, $item_key, $controller)?>"><img src="<?=$cp_theme_url?>images/external_link.png"/></a></td>
						<?php else:?>
