...
<?php endif;?>
		...
		<?=form_open_multipart('C=content_publish'.AMP.'M=entry_form'.$BK, $form_additional, $form_hidden)?>
			...
			<ul class="tab_menu" id="tab_menu_tabs">
				<?php foreach($publish_tabs as $tab => $field_list):?><li id="menu_<?=url_title($tab, 'underscore', TRUE)?>" class="content_tab"><a href="#" title="menu_<?=url_title($tab, 'underscore', TRUE)?>" class="menu_<?=url_title($tab, 'underscore', TRUE)?>"><?=ucfirst(lang($tab))?></a>&nbsp;</li><?php endforeach; if ($this->session->userdata('group_id') == 1):?><li class="addTabButton"><a class="add_tab_link" href="#"><img src="<?=$cp_theme_url?>images/add_publish_tab_button.gif" alt="<?=lang('add_tab')?>" width="10" height="10"/><?=lang('add_tab')?></a>&nbsp;</li><?php endif?>
			</ul>
