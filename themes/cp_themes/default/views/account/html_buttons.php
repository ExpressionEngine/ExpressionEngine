...
<div>
	...
	<?=form_open('C=myaccount'.AMP.'M=html_buttons', '', $form_hidden)?>
	...
	else
	{
		foreach($html_buttons->result() as $button)
		{
			...
			$this->table->add_row(
								...
								form_input(array('id'=>"tag_name_{$i}",'name'=>"tag_name_{$i}",'value'=>$button->tag_name)).
								form_label(lang('tag_name'), "tag_name_{$i}", array('class'=>'html_button_label')),

								form_input(array('id'=>"tag_open_{$i}",'name'=>"tag_open_{$i}",'value'=>$button->tag_open)).
								form_label(lang('tag_open'), "tag_open_{$i}", array('class'=>'html_button_label')),
						
								form_input(array('id'=>"tag_close_{$i}",'name'=>"tag_close_{$i}",'value'=>$button->tag_close)).
								form_label(lang('tag_close'), "tag_close_{$i}", array('class'=>'html_button_label')),

								form_input(array('id'=>"accesskey_{$i}",'name'=>"accesskey_{$i}",'value'=>$button->accesskey, 'size'=>10)).
								...
								array(
									'data'=>'<img src="'.$cp_theme_url.'images/drag.png" />'.
										...
										form_input(array('id'=>"tag_order_{$i}",'name'=>"tag_order_{$i}",'value'=>$button->tag_order, 'size'=>5)).
										form_label(lang('tag_order'), "tag_order_{$i}", array('class'=>'html_button_label')),
									'class'=>'tag_order'
								),
							
								array(
									'data'=>'<a href="'.BASE.AMP.'C=myaccount'.AMP.'M=delete_html_button'.AMP.'button_id='.$button->id.'"><img src="'.$cp_theme_url.'images/content_custom_tab_delete.png" alt="'.lang('delete').'" width="19" height="18" /></a>', 
									'class'=>'del_row'
								)
							);
		}
