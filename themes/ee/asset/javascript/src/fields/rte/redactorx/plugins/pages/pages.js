RedactorX.add('plugin', 'pages', {
    ...
    subscribe: {
        ...
        'editor.keyup': function(event) {
            if (typeof(this.opts.handle) === 'undefined') return;
            this._handle(event);
        }
    },
    start: function() {
    ...
    _handle: function(event) {
        ...
        if (key === ks.BACKSPACE) {
            ...
            if (this.handleLen <= this.opts.pages.start) {
                this._hide();
            }
        }
    ...
    _emit: function() {
        var re = new RegExp('^' + this.opts.pages.trigger);
        ...
        if (re.test(this.handleStr)) {
            if (this.handleStr2 && this.handleStr2[0] === ' ' || this.handleStr2[0] === '') {
                this.handleStr = this.handleStr.replace(this.opts.pages.trigger, '');
                this.handleLen++;

                if ((this.handleLen-1) > this.opts.pages.start) {
                    this._load();
                }
            }
        }
    ...
    _build: function(data) {
        ...
        for (var key in data) {
            ...
            $trigger.html(this.data[key].text);
            $trigger.attr('data-href', this.data[key].href);
            $trigger.attr('data-text', this.data[key].text);
    ...
    _replace: function(e, $el) {
        ...
        var re = new RegExp(this.opts.pages.trigger + this.handleStr + '$');
