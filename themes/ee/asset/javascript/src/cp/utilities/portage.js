...
EE.cp.portageImport = {
	...
	init: function() {
		...
		EE.cp.portageImport._init();
		$('button.toggle-btn').on('click', function(e) {

			$('fieldset:first em').text(EE.portageImport.desc.replace('%s', EE.portageImport.components));
		});
	},
	 ...
	 */
	_sendAjaxRequest: function(progress, step = 'load') {
		...
		if (progress !== undefined) {
			data = {
				progress: progress,
				step: step,
                components: EE.portageImport.components
			};
		}
		...
		request.open('POST', EE.portageImport.endpoint, true);
			...
			if (request.status >= 200 && request.status < 400) {
				...
				if (response.status == 'error') {
					...
					if (typeof(response.redirect) !== 'undefined') {
						window.location = EE.portageImport.base_url;
						return;
					}
					return;
				}
				...
				that._sendAjaxRequest(response.progress, response.step);
			} else {
				if (response.status == 'error') {
					that._presentError(response.message);
					return;
	...
	_getPercentageForResponse: function(response) {
		var progress = 0,
			total_fields = EE.portageImport.components.length * 3;
	...
	_presentError: function(text) {
		var alert = EE.portageImport.ajax_fail_banner.replace('%body%', text),
...
if (document.readyState != 'loading') {
	EE.cp.portageImport.init();
} else {
	document.addEventListener('DOMContentLoaded', EE.cp.portageImport.init);
}
