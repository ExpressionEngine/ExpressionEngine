$(document).ready(function(){function e(e){return e+="",2==e.length?e:"0"+e}function t(t,a){var i=t.getFullYear(),r=t.getMonth()+1,l=t.getDate(),d=t.getDay(),s=t.getHours(),c=t.getMinutes();s=(s+11)%12+1,1==l?suffix="st":2==l?suffix="nd":3==l?suffix="rd":suffix="th",diff=t-new Date(t.getFullYear(),0,0),doy=Math.ceil(diff/864e5)-1,2==r?1==new Date(i,1,29).getMonth()?days_in_month=29:days_in_month=28:[4,6,9,11].indexOf(r)>-1?days_in_month=30:days_in_month=31;var o={
// Day
...
// Month
F:EE.lang.date.months.full[r-1],m:e(r),M:EE.lang.date.months.abbreviated[r-1],n:r,t:days_in_month,
// Year
L:1==new Date(i,1,29).getMonth()?1:0,
// o: year,
Y:i,y:t.getFullYear().toString().substr(2,2),
// Time
...
var n=/%d|%D|%j|%l|%N|%S|%w|%z|%W|%F|%m|%M|%n|%t|%L|%o|%Y|%y|%a|%A|%B|%g|%G|%h|%H|%i|%s|%u|%e|%I|%O|%P|%T|%Z|%c|%r|%U|"[^"]*"|'[^']*'/g,i={calendars:[],element:null,
// showing
...
$(".date-picker-clip-inner").on("click",".date-picker-next",function(e){r.next(),
...
$(".date-picker-clip-inner").on("click",".date-picker-prev",function(e){r.prev(),
...
e.preventDefault()}),
// listen for clicks on elements classed with .date-picker-back
$(".date-picker-clip-inner").on("click",".date-picker-item td a",function(e){if($(".date-picker-item td.act").removeClass("act"),$(this).closest("td").addClass("act"),$(i.element).val()){var a=new Date(1e3*$(i.element).attr("data-timestamp"));a.setYear(i.year),a.setMonth(i.month),a.setDate($(this).text())}else var a=new Date(i.year,i.month,$(this).text());var n=new Date;a.setHours(n.getHours()),a.setMinutes(n.getMinutes()),a.setSeconds(n.getSeconds()),$(i.element).val(t(a,EE.date.date_format)),$(i.element).attr("data-timestamp",t(a,"%U")),$(i.element).focus(),$(".date-picker-wrap").toggle(),e.preventDefault()}),
// Prevent manual scrolling of the huge inner clip div
$(".date-picker-clip-inner").on("mousewheel",function(e){e.preventDefault()})}if($(this.element).val()){var c=$(this.element).attr("data-timestamp");a=c?new Date(1e3*c):new Date(Date.parse($(this.element).val())),n=a.getUTCDate(),l=a.getUTCFullYear(),d=a.getUTCMonth()}else a=new Date,l=a.getFullYear(),d=a.getMonth();var o=this.generate(l,d);null!=o&&($(".date-picker-clip-inner").html(o),n&&$(".date-picker-item td:contains("+n+")").each(function(){$(this).text()==n&&$(this).addClass("act")}))},generate:function(e,t){if(
// Set variables
this.month=t,this.year=e,i.calendars.indexOf(e+"-"+t)>-1)return null;var a=r.total_days(e,t),n=(r.total_days(e,t-1),r.first_day(e,t)),l=7-(n+a)%7,d=t-1>-1?t-1:11,s=12>t+1?t+1:0;l=7==l?0:l;
// Leading dimmed
...
// Trailing dimmed
for(var m=0;l>m;m++)p[u++]='<td class="empty"></td>',h++;return p[u++]="</tr>",this.calendars.push(e+"-"+t),c.join("")+p.join("")+o.join("")}},r={select:function(e){var t=new Date(i.year,e);return i.generate(t.getFullYear(),t.getMonth())},prev:function(){var e=this.select(i.month-1);if(null!=e){$(".date-picker-clip-inner").prepend(e);var t=$(".date-picker-clip").scrollLeft();$(".date-picker-clip").scrollLeft(t+260)}},next:function(){var e=this.select(i.month+1);null!=e&&$(".date-picker-clip-inner").append(e)},total_days:function(e,t){return 32-new Date(e,t,32).getDate()},first_day:function(e,t){return new Date(e,t,1).getDay()}};a($('input[rel="date-picker"]').not(".grid-input-form input")),
