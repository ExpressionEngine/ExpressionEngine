...
(function($) {
     ...
     * Artee
     */
    Artee = function(id, config, defer) {
        ...
        this.config = (EE.Artee.configs[config] || EE.Artee.configs['default']);
    ...
    Artee.prototype = {
        ...
        showIframe: function() {
            ...
            this.$iframe = $('<iframe class="artee" style="width:'+width+'; height:'+height+';" frameborder="0" />').insertAfter($textarea);
    ...
    if (typeof FluidField !== 'undefined') {
        FluidField.on('artee', 'add', function(row) {
            var field_id = row.find('.artee-textarea').attr('id');
            var config_handle = $('#'+field_id).data('config');
            var defer = $('#'+field_id).data('defer');

            if(defer === 'n') defer = false;

            new Artee(field_id, config_handle, defer);
        });
    }
     ...
     */
    Artee.loadEEFileBrowser = function(params, directory, content_type) {
        ...
        var $trigger = $('<trigger class="m-link filepicker" rel="modal-file" href="' + EE.Artee.fpUrl + '"/>').appendTo('body');
...
var artee_pages_loading = null;
function getPages( queryText ) {
    if (artee_pages_loading) {
        artee_pages_loading.abort();
    }
    //we only make request if the previous one had some results
    return new Promise( resolve => {
        var data = {search: queryText};
        artee_pages_loading = $.ajax({
            url: EE.Artee.pages_autocomplete,
            type: "GET",
            data: data,
            cache: true,
