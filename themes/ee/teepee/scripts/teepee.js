...
(function($) {
     ...
     * Teepee
     */
    Teepee = function(id, config, defer) {
        ...
        this.config = (EE.Teepee.configs[config] || EE.Teepee.configs['default']);
    ...
    Teepee.prototype = {
        ...
        showIframe: function() {
            ...
            this.$iframe = $('<iframe class="teepee" style="width:'+width+'; height:'+height+';" frameborder="0" />').insertAfter($textarea);
    ...
    if (typeof FluidField !== 'undefined') {
        FluidField.on('teepee', 'add', function(row) {
            var field_id = row.find('.teepee-textarea').attr('id');
            var config_handle = $('#'+field_id).data('config');
            var defer = $('#'+field_id).data('defer');

            if(defer === 'n') defer = false;

            new Teepee(field_id, config_handle, defer);
        });
    }
     ...
     */
    Teepee.loadEEFileBrowser = function(params, directory, content_type) {
        ...
        var $trigger = $('<trigger class="m-link filepicker" rel="modal-file" href="' + EE.Teepee.fpUrl + '"/>').appendTo('body');
...
function getPages( queryText ) {
    ...
    return new Promise( resolve => {
        ...
        teepee_pages_loading = $.ajax({
            url: EE.Teepee.pages_autocomplete,
            type: "GET",
            data: data,
            cache: true,
