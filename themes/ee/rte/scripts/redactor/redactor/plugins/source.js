(function($)
{
	$.Redactor.prototype.source = function()
	{
		return {
			...
			toggle: function()
			{
				return (this.source.$textarea.attr('data-state') == 'open') ? this.source.hide() : this.source.show();
			},
			...
			hide: function()
			{
				this.source.$textarea.attr('data-state', 'hidden').hide();
			...
			show: function()
			{
				...
				this.source.$textarea.val(code).height(height).attr('data-state', 'open').show();
