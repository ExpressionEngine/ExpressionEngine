...
<html>
	...
	<body id="top">
		...
		<section class="bar info-wrap">
			<nav class="snap">
				<div class="site">
					<a class="home" href="<?=ee('CP/URL')->make('homepage')?>"></a>
					<?php if (ee()->config->item('multiple_sites_enabled') === 'y'): ?>
						...
						<ul class="sites-list sub-menu">
							...
							<?php if (ee()->cp->allowed_group('can_admin_sites')): ?>
								<a class="last add" href="<?=ee('CP/URL')->make('msm/create')?>"><?=lang('new_site')?></a>
							<?php endif ?>
						</ul>
					<?php elseif ( ! ($site_name = ee()->config->item('site_name')) OR empty($site_name)): ?>
						<a href="<?=ee('CP/URL')->make('settings/general')?>" class="no-name"><?=lang('name_your_site')?></a>
					<?php else: ?>
						<a href="<?=ee()->config->item('site_url')?>" rel="external"><?=ee()->config->item('site_name')?></a>
					<?php endif ?>
				</div>
				<div class="user">
					<a href="<?=ee('CP/URL')->make('login/logout')?>"><?=lang('log_out')?></a> <a class="has-sub" href=""><?=$cp_screen_name?></a>
		...
		<section class="bar menu-wrap">
			<nav class="snap">
				<ul class="author-menu">
					<li>
						...
						<div class="sub-menu">
							...
							<ul class="channels-create">
								...
								<?php endforeach ?>
								<li class="last"><a class="add" href="<?=ee('CP/URL')->make('channels/create')?>"><?=lang('new_channel')?></a></li>
							</ul>
						</div>
					</li>
					<li>
						...
						<div class="sub-menu">
							...
							<ul class="channels-edit">
								<li class="search-channel" data-search="<?= strtolower(lang('view_all')) ?>">
									<a href="<?=ee('CP/URL')->make('publish/edit')?>"><?= lang('view_all') ?></a>
								</li>
					...
					<li><a href="<?=ee('CP/URL')->make('files')?>"><?=lang('menu_files')?></a></li>
					<li><a href="<?=ee('CP/URL')->make('members')?>"><?=lang('menu_members')?></a></li>
				</ul>
				<ul class="dev-menu">
					<li class="develop">
					...
					<li class="settings"><a href="<?=ee('CP/URL')->make('settings/general')?>"><b class="ico settings"></b> <!-- Settings --></a></li>
				</ul>
			</nav>
		</section>
		<section class="wrap">
