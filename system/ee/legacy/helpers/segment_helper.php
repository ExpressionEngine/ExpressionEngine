...
{
    if ($dynamic && preg_match("#(^|\/)(\d{4}/\d{2}/\d{2})#", (string) $qstring, $match)) {
        $ex = explode('/', $match[2]);
...
function parse_year_month($qstring, $dynamic = true)
{
    ...
    if ($dynamic && preg_match("#(^|\/)(\d{4}/\d{2})(\/|$)#", (string) $qstring, $match)) {
        $ex = explode('/', $match[2]);
...
function parse_category($query_string)
{
    ...
    if ($reserved_category_word != ''
        && strpos((string) $query_string, $reserved_category_word) !== false
    ) {
        $split = explode('/', (string) $query_string);
    ...
    elseif (preg_match("#(^|\/)C(\d+)(\/|$)#", (string) $query_string, $match)) {
        return $match[2];
    }
