...
<figure class="file-chosen<?php if ( ! $file) echo " hidden";?>">
	<img id="<?=$field_name?>" src="<?=$file->getThumbnailURL()?>" alt="<?=$file->title?>">
	<ul class="toolbar">
...
<?php endif; ?>
<p class="solo-btn<?php if ($file) echo " hidden";?>"><a class="btn action m-link filepicker" href="<?=$fp_url?>" rel="modal-file" data-input-image="<?=$field_name?>" data-input-value="<?=$field_name?>"><?=lang('upload_file')?></a></p>