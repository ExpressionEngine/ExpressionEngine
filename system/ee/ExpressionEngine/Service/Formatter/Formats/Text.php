...
class Text extends Formatter
{
    ...
    public function accentsToAscii()
    {
        ...
        $chars = preg_split('//u', (string) $this->content, 0, PREG_SPLIT_NO_EMPTY);
    ...
    {
        $this->content = htmlspecialchars((string) $this->content, ENT_QUOTES, 'UTF-8', $double_encode);
    ...
    public function attributeSafe($options = [])
    {
        ...
        if (strpos((string) $this->content, '<div class="codeblock">') !== false) {
            $this->content = preg_replace('|<div class="codeblock">.*?</div>|is', '', (string) $this->content);
        }
        ...
        $this->content = strip_tags((string) $this->content);
    ...
    public function censor()
    {
        ...
        if (! is_array($censored)) {
            ...
            $censored = preg_split('/[\n|\|]/', (string) $censored, 0, PREG_SPLIT_NO_EMPTY);
        ...
        foreach ($censored as $bad) {
            if ($replace) {
                $this->content = preg_replace($bad, (string) $replace, (string) $this->content);
            } else {
    ...
    {
        $this->content = htmlentities((string) $this->content, ENT_QUOTES, 'UTF-8');
    ...
    public function emojiShorthand()
    {
        ...
        if (substr_count((string) $this->content, ':') < 2) {
            return $this;
        }
        ...
        // grab 'em!
        preg_match_all($shorthand_regex, (string) $this->content, $matches, PREG_SET_ORDER);
        ...
        $use_regex = (stripos((string) $this->content, '[code') !== false or stripos((string) $this->content, '<code') !== false);
                ...
                if ($usingLegacyEmojiConfig) {
                    if ($use_regex) {
                        ...
                        $this->content = preg_replace(
                            ...
                            (string) $emoji_map[$match[1]]->html_entity,
                            (string) $this->content
                        );
                    } else {
    ...
    public function encodeEETags($options = [])
    {
        ...
        if ($this->content != '' && strpos((string) $this->content, '{') !== false) {
            if ($encode_vars === true) {
    ...
    public function limitChars($options = [])
    {
        ...
        if ($preserve_words) {
            ...
            $this->content = wordwrap((string) $this->content, $limit, "\n", true);
        ...
        $this->content = (strlen((string) $cut) == strlen((string) $this->content)) ? $cut : $cut . $end_char;
    ...
    public function replace($options = [])
    {
        ...
        if ($valid !== false or DEBUG) {
            $this->content = preg_replace($find, $replace, (string) $this->content);
        }
    ...
    public function trim($options = [])
    {
        ...
        $this->content = isset($options['characters'])
            ? trim((string) $this->content, $options['characters'])
    ...
    public function url()
    {
        ...
        if (strncasecmp((string) $this->content, 'http', 4) === 0 && strlen((string) $this->content) <= 8) {
            $this->content = '';

            return $this;
        }

        $url = parse_url((string) $this->content);
    ...
    public function urlDecode($options = [])
    {
        if (isset($options['plus_encoded_spaces']) && get_bool_from_string($options['plus_encoded_spaces'])) {
            $this->content = urldecode((string) $this->content);
        } else {
            $this->content = rawurldecode((string) $this->content);
        }
    ...
    {
        if (isset($options['plus_encoded_spaces']) && get_bool_from_string($options['plus_encoded_spaces'])) {
            $this->content = urlencode((string) $this->content);
        } else {
            $this->content = rawurlencode((string) $this->content);
        }
    ...
    public function urlSlug($options = [])
    {
        ...
        $this->content = trim(stripslashes((string) $this->content), '-_.');
        ...
        if (isset($options['remove_stopwords']) && get_bool_from_string($options['remove_stopwords'])) {
            ...
            foreach ($stopwords as $stopword) {
                $this->content = preg_replace("/\b" . preg_quote((string) $stopword, '/') . "\b/iu", "", (string) $this->content);
            }

            // reduce any multiples this left behind, and any end bits
            $this->content = preg_replace('#' . $options['separator'] . '+#i', $options['separator'], (string) $this->content);
            $this->content = trim(stripslashes((string) $this->content), '-_.');
        }
