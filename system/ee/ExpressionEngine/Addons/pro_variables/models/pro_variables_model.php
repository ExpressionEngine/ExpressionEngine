...
/**
 * Pro Variables Model class
 */
abstract class Pro_variables_model extends CI_Model
{
    // --------------------------------------------------------------------
    // PROPERTIES
    // --------------------------------------------------------------------
    ...
    protected function initialize($table, $pk, $attributes)
    {
        ...
        if (substr((string) $table, 0, strlen((string) $prefix)) != $prefix) {
            $table = $prefix . $table;
        }
...
// End of file model.pro_variables.php
