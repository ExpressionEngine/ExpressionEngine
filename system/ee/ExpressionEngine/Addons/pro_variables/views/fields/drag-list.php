<?php 
    if (isset($var_type) && $var_type == 'Select Entries'):
        $component = [
            ...
            'no_results' => strip_tags((string) lang('no_entries_found')),
            'button_label' => null,
            'can_add_items' => false,
    ...
    <ul class="pro-off">
        <?php foreach ($choices as $key => $val) : ?>
            <?php if (! in_array($key, (array) $value)) : ?>
                <li><input type="hidden" value="<?=htmlspecialchars((string) $key, ENT_QUOTES)?>"><?=$val?></li>
            <?php endif; ?>
        <?php endforeach; ?>
    </ul>
    <ul class="pro-on">
        <?php foreach ($value as $key) : ?>
            <?php if (array_key_exists((string) $key, $choices)) : ?>
                <li><input type="hidden" name="<?=$name?>[]" value="<?=htmlspecialchars((string) $key, ENT_QUOTES)?>"><?=$choices[$key]?></li>
            <?php endif; ?>
        <?php endforeach; ?>
    </ul>
