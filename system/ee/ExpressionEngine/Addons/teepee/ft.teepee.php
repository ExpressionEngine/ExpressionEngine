...
class Teepee_ft extends EE_Fieldtype
{
    ...
    {
        $settings = ee('Request')->post('teepee');
    ...
    public function grid_display_field($data)
    {
        ...
        if (! isset($cache['displayed_grid_cols'])) {
            ee()->cp->add_to_foot('<script type="text/javascript" src="' . URL_THEMES . 'teepee/scripts/grid.js"></script>');
        ...
        if (! isset($cache['displayed_grid_cols'][$this->settings['col_id']])) {
            ...
            ee()->javascript->output('Teepee.gridColConfigs.col_id_' . $this->settings['col_id'] . ' = ["' . $configHandle . '", ' . $defer . '];');
    ...
    public function var_display_field($data)
    {
        ...
        $this->settings = array_merge($this->settings, TeepeeHelper::getGlobalSettings());
    ...
    public function pre_process($data)
    {
        ...
        // convert site page tags to URLs
        TeepeeHelper::replacePageTags($data, $entrySiteId);
    ...
    public function replace_tag($data, $params = array(), $tagdata = false)
    {
        ...
        if (ee()->extensions->active_hook('teepee_before_replace')) {
            $data = ee()->extensions->call('teepee_before_replace', $this, $data);
        }
        ...
        if (ee()->extensions->active_hook('teepee_before_replace_end')) {
            $data = ee()->extensions->call('teepee_before_replace_end', $this, $data);
        }
    ...
    private function _fieldSettings($settings)
    {
        $settings = array_merge([
            'toolset_id' => ee()->config->item('teepee_default_toolset_id'),
            'defer'     => 'n'
        ], $settings);

        // load the language file
        ee()->lang->loadfile('teepee');

        $configModels = ee('Model')->get('teepee:Toolset')->all();
        $configOptions = array();
        ...
        if (!empty($configOptions)) {
            $configFields = array(
                ...
                array(
                    'type'    => 'html',
                    'content' => '(<a href="' . ee('CP/URL')->make('addons/settings/teepee')->compile() . '">' . lang('teepee_edit_configs') . '</a>)'
                )
            );
        } else {
            $configFields = array(
                array(
                    'type'    => 'html',
                    'content' => '<a href="' . ee('CP/URL')->make('addons/settings/teepee/editConfig')->compile() . '">' . lang('teepee_create_config') . '</a>'
                )
            );
        }
        ...
        $settings = array(
            array(
                'title' => lang('teepee_editor_config'),
                'fields' => $configFields
            ),
            array(
                'title' => lang('teepee_defer'),
                'fields' => array(
                    'teepee[defer]' => array(
                        'type' => 'yes_no',
