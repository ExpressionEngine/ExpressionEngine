...
use ExpressionEngine\Addons\Artee\Model\Toolset;
use ExpressionEngine\Library\CP\Table;
use ExpressionEngine\Library\Rte\RteFilebrowserInterface;

/**
 * Artee Module Control Panel Class
 *
 * @package   Artee
...
class Artee_mcp
{
    ...
    {
        $this->base_url = ee('CP/URL')->make('addons/settings/artee');
    }
    ...
    {
        $toolsets = ee('Model')->get('artee:Toolset')->all();
        ...
        if (ee('Request')->isPost()) {
            $rules = array(
                'artee_default_toolset_id' => 'required|enum[' . implode(',', $toolset_ids) . ']',
                'artee_file_browser' => 'required|enum[' . implode(',', array_keys($file_browser_choices)) . ']'
            );
            $validationResult = ee('Validation')->make($rules)->validate($_POST);

            if ($validationResult->passed()) {
                $prefs = [
                    'artee_default_toolset_id' => ee()->input->post('artee_default_toolset_id'),
                    'artee_file_browser' => ee()->input->post('artee_file_browser')
                ];
        ...
        if (empty($prefs)) {
            $prefs = [
                'artee_default_toolset_id' => ee()->config->item('artee_default_toolset_id') ? ee()->config->item('artee_default_toolset_id') : reset($toolset_ids),
                'artee_file_browser' => ee()->config->item('artee_file_browser') ? ee()->config->item('artee_file_browser') : reset($file_browser_choices)
            ];
        }
        ...
        foreach ($toolsets as $t) {
            ...
            $url = ee('CP/URL')->make('addons/settings/artee/edit_toolset', array('toolset_id' => $t->toolset_id));
            $checkbox = array(
                'name' => 'selection[]',
                'value' => $t->toolset_id,
                'data' => array(
            ...
            if ($prefs['artee_default_toolset_id'] == $t->toolset_id) {
            ...
            $toolset = array(
                ...
                array('toolbar_items' => array(
                        ...
                        'copy' => array(
                            'href' => ee('CP/URL')->make('addons/settings/artee/edit_toolset', array('toolset_id' => $t->toolset_id, 'clone' => 'y')),
                            'title' => lang('clone'),
                        )
                    )
                ),
        ...
        $vars = array(
            'cp_page_title' => lang('artee_module_name') . ' ' . lang('configuration'),
            ...
            'sections' => array(
                array(
                    array(
                        ...
                        'desc' => '',
                        'fields' => array(
                            'artee_default_toolset_id' => array(
                                'type' => 'radio',
                                'choices' => $toolset_opts,
                                'value' => $prefs['artee_default_toolset_id'],
                                'no_results' => [
                    ...
                    array(
                        'title' => 'artee_file_browser',
                        'desc' => 'artee_file_browser_desc',
                        'fields' => array(
                            'artee_file_browser' => array(
                                'required' => true,
                                'type'     => 'select',
                                'value' => $prefs['artee_file_browser'],
        ...
        // return the page
        return ee('View')->make('artee:index')->render($vars);
    }
    ...
    public function edit_toolset()
    {
        ...
        $defaultConfigSettings = ArteeHelper::defaultConfigSettings();

        if (
            ($toolset_id = $request->get('toolset_id'))
            && ($config = ee('Model')->get('artee:Toolset')->filter('toolset_id', '==', $toolset_id)->first())
        ) {
            /**
             * @var $config Config
             */
            ...
            if ($request->get('clone') == 'y') {
                $config->toolset_id = '';
                $config->toolset_name .= ' ' . lang('artee_clone');
                $headingTitle = lang('artee_create_config');
            } else {
                $headingTitle = lang('artee_edit_config') . ' - ' . $config->toolset_name;
            }
        } else {
            $config = ee('Model')->make('artee:Toolset', array(
                'toolset_id' => '',
                'toolset_name' => '',
        ...
        $fileBrowserOptions = array_unique([ee()->config->item('artee_file_browser'), 'filepicker']);
        ...
        $fullToolbar = ArteeHelper::defaultToolbars()['Full'];
        $fullToolset = array();
        ...
        $variables['base_url'] = ee('CP/URL')->make('addons/settings/artee/saveConfig');
        ...
        return ee('View')->make('artee:form')->render($variables);
    }
    ...
    public function saveConfig()
    {
        ...
        /**
         * @var $config \EEHarbor\Artee\Model\Config
         */
        ...
        ee('CP/Alert')->makeInline('shared-form')
            ->asSuccess()
            ->withTitle(lang('artee_config_saved'))
            ->addToBody(sprintf(lang('artee_config_saved_desc'), $configName))
            ->defer();
    ...
    public function deleteConfig()
    {
        ...
        if (!empty($toolset_id)) {
            ...
            if ($config) {
                ...
                ee('CP/Alert')->makeInline('shared-form')
                    ->asSuccess()
                    ->withTitle(lang('artee_config_deleted'))
                    ->addToBody(sprintf(lang('artee_config_deleted_desc'), $config->toolset_name))
                    ->defer();
            }
        }
