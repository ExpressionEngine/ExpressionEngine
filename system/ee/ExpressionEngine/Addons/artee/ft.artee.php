...
class Artee_ft extends EE_Fieldtype
{
    ...
    {
        $settings = ee('Request')->post('artee');
    ...
    {
        ArteeHelper::includeFieldResources();
        $configHandle = ArteeHelper::insertConfigJsById(!empty($this->settings['toolset_id']) ? $this->settings['toolset_id'] : null);
        ...
        if (strpos($id, '_new_') === false) {
            ee()->cp->add_to_foot('<script type="text/javascript">new Artee("' . $id . '", "' . $configHandle . '", ' . $defer . ');</script>');
        }
        ...
        ArteeHelper::replaceFileTags($data);

        // convert site page tags to URLs
        ArteeHelper::replacePageTags($data);

        //Third party conversion
        ArteeHelper::replaceExtraTags($data);

        if (ee()->extensions->active_hook('artee_before_display')) {
            $data = ee()->extensions->call('artee_before_display', $this, $data);
        }
    ...
    public function grid_display_field($data)
    {
        ...
        // get the cache
        if (! isset(ee()->session->cache['artee'])) {
            ee()->session->cache['artee'] = array();
        }
        $cache =& ee()->session->cache['artee'];

        if (! isset($cache['displayed_grid_cols'])) {
            ee()->cp->add_to_foot('<script type="text/javascript" src="' . URL_THEMES . 'artee/scripts/grid.js"></script>');
        ...
        if (! isset($cache['displayed_grid_cols'][$this->settings['col_id']])) {
            ...
            ee()->javascript->output('Artee.gridColConfigs.col_id_' . $this->settings['col_id'] . ' = ["' . $configHandle . '", ' . $defer . '];');
    ...
    public function var_display_field($data)
    {
        ...
        $this->settings = array_merge($this->settings, ArteeHelper::getGlobalSettings());
    ...
    public function save($data)
    {
        ...
        ArteeHelper::replaceFileUrls($data);

        // Convert page URLs to tags
        ArteeHelper::replacePageUrls($data);

        if (ee()->extensions->active_hook('artee_before_save')) {
            $data = ee()->extensions->call('artee_before_save', $this, $data);
        }
    ...
    public function pre_process($data)
    {
        ...
        // convert site page tags to URLs
        ArteeHelper::replacePageTags($data, $entrySiteId);
    ...
    public function replace_tag($data, $params = array(), $tagdata = false)
    {
        ...
        if (ee()->extensions->active_hook('artee_before_replace')) {
            $data = ee()->extensions->call('artee_before_replace', $this, $data);
        }
        ...
        if (ee()->extensions->active_hook('artee_before_replace_end')) {
            $data = ee()->extensions->call('artee_before_replace_end', $this, $data);
        }
    ...
    private function _fieldSettings($settings)
    {
        $settings = array_merge([
            'toolset_id' => ee()->config->item('artee_default_toolset_id'),
            'defer'     => 'n'
        ], $settings);

        // load the language file
        ee()->lang->loadfile('artee');

        $configModels = ee('Model')->get('artee:Toolset')->all();
        $configOptions = array();
        ...
        if (!empty($configOptions)) {
            $configFields = array(
                ...
                array(
                    'type'    => 'html',
                    'content' => '(<a href="' . ee('CP/URL')->make('addons/settings/artee')->compile() . '">' . lang('artee_edit_configs') . '</a>)'
                )
            );
        } else {
            $configFields = array(
                array(
                    'type'    => 'html',
                    'content' => '<a href="' . ee('CP/URL')->make('addons/settings/artee/editConfig')->compile() . '">' . lang('artee_create_config') . '</a>'
                )
            );
        }
        ...
        $settings = array(
            array(
                'title' => lang('artee_editor_config'),
                'fields' => $configFields
            ),
            array(
                'title' => lang('artee_defer'),
                'fields' => array(
                    'artee[defer]' => array(
                        'type' => 'yes_no',
