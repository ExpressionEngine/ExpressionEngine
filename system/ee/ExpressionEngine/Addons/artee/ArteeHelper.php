...
class ArteeHelper
{
    ...
    /**
     * Includes the necessary CSS and JS files to get Artee fields working.
     */
    public static function includeFieldResources()
    {
        if (! static::$_includedFieldResources) {
            ee()->cp->add_to_foot('<script type="text/javascript" src="' . URL_THEMES . 'artee/scripts/ckeditor.js"></script>');
            ee()->cp->add_to_foot('<script type="text/javascript" src="' . URL_THEMES . 'artee/scripts/artee.js"></script>');
            ee()->cp->add_to_head('<link rel="stylesheet" type="text/css" href="' . URL_THEMES . 'artee/styles/artee.css' . '" />');
            ...
            ee()->javascript->set_global([
                'Artee.pages_autocomplete' => ee()->functions->fetch_site_index(0, 0) . QUERY_MARKER . 'ACT=' . $action_id->row('action_id') . '&t=' . ee()->localize->now,
                'Artee.filedirUrls' => $filedir_urls
            ]);
    ...
    /**
     * Inserts the Artee config JS in the page foot by config ID.
     *
     * @param $configId
     *
     * @return $configHandle The handle for config used by Artee JS
     */
    public static function insertConfigJsById($configId)
    {
        ...
        if ($toolset = ee('Model')->get('artee:Toolset')->filter('toolset_id', $configId)->first()) {
            $configHandle = preg_replace('/[^a-z0-9]/i', '_', $toolset->toolset_name) . $configId;
            $config = array_merge($baseConfig, $toolset->settings);
        } elseif ($toolset = ee('Model')->get('artee:Toolset')->filter('toolset_id', ee()->config->item('artee_default_toolset_id'))->first()) {
            $configHandle = preg_replace('/[^a-z0-9]/i', '_', $toolset->toolset_name) . ee()->config->item('artee_default_toolset_id');
            $config = array_merge($baseConfig, $toolset->settings);
        } else {
            $config = $baseConfig;
        ...
        if (!empty(ee()->config->item('site_pages'))) {
            ee()->cp->add_to_foot('<script type="text/javascript">
                EE.Artee.configs.' . $configHandle . '.mention = {"feeds": [{"marker": "@", "feed": getPages, "itemRenderer": formatPageLinks, "minimumCharacters": 3}]};
            </script>');
        }
        ...
        // -------------------------------------------
        ee()->javascript->set_global([
            'Artee.configs.' . $configHandle => $config
        ]);
    ...
    public static function getSitePages($search = '', $site_id = null)
    {
        ...
        if ($pages === false) {
            $break = false;
            /**
             * `artee_autocomplete_pages` extension hook
             ...
             *      ];
             */
            if (ee()->extensions->active_hook('artee_autocomplete_pages') === true) {
                $pages = ee()->extensions->call('artee_autocomplete_pages', $pages, $search, $site_id);
