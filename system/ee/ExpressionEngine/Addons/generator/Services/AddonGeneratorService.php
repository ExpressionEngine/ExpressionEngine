...
require PATH_THIRD . '/vendor/autoload.php';

use Generator\Enums\FieldtypeCompatibility;
use Generator\Enums\Hooks;
...
class AddonGeneratorService {
	...
	private function init()
	{
		...
		$this->generatorPath = SYSPATH . 'ee/EllisLab/Addons/generator';
		$this->addonPath = SYSPATH . 'ee/user/addons/' . $this->slug;

		ee()->load->library('filemanager');

		// Get stub path
		$this->stubPath = PATH_THIRD . ('stubs') . '/';

		// Create temp directory
		$tempDir = Str::random();
		File::makeDirectory(storage_path() . '/' . $tempDir);
		$this->tempPath = storage_path($tempDir) . '/';
	...
	protected function buildFieldtype()
	{
		...
		$stub = File::get($this->stub('ft.slug.php'));
	...
	protected function buildExtension()
	{
		...
		$stub = File::get($this->stub('ext.slug.php'));
		$stub = $this->write('slug_uc', $this->slug_uc, $stub);
		$stub = $this->write('version', $this->version, $stub);
		...
		foreach ($this->hooks as $hook) {
			...
			$hookArrayStub = File::get($this->stub('hook_array.php'));
			$hookArrayStub = $this->write('hook_name', $hook, $hookArrayStub);
			$hook_array .= "{$hookArrayStub}\n";

			$hookMethodStub = File::get($this->stub('hook_method.php'));
	...
	protected function buildModule()
	{
		...
		// Create upd file
		$stub = File::get($this->stub('upd.slug.php'));
		$stub = $this->write('slug_uc', $this->slug_uc, $stub);
		$stub = $this->write('version', $this->version, $stub);
		...
		if($this->hooks) {
			...
			$hookInstall = File::get($this->stub('hook_install.php'));
			...
			$hooksUninstall = File::get($this->stub('hook_uninstall.php'));
		...
		// Create module file
		$stub = File::get($this->stub('mod.slug.php'));
		$stub = $this->write('slug_uc', $this->slug_uc, $stub);
		$this->putFile('mod.' . $this->slug . '.php', $stub);

		// Create control panel file
		$stub = File::get($this->stub('mcp.slug.php'));
		$stub = $this->write('slug_uc', $this->slug_uc, $stub);
		$stub = $this->write('slug', $this->slug, $stub);
		$this->putFile('mcp.' . $this->slug . '.php', $stub);

		// Create lang file
		File::makeDirectory($this->tempPath . 'language');
		File::makeDirectory($this->tempPath . 'language/english');
		$stub = File::get($this->stub('slug_lang.php'));
		$stub = $this->write('name', $this->name, $stub);
	...
	protected function buildPlugin()
	{
		...
		$stub = File::get($this->stub('pi.slug.php'));
	...
	private function buildAddonSetup()
	{
		...
		$stub = File::get($this->stub('addon.setup.php'));
		...
		if($this->type == 'fieldtype') {
			$ftSetup = File::get($this->stub('fieldtype_setup.php'));
		...
		if(Arr::has($this->data, 'services') && ($services = Arr::get($this->data, 'services'))) {
			...
			File::makeDirectory($this->tempPath . 'Services');

			foreach ($services as $service) {

				if(!$service || $service == '') continue;

				$servicesStub = File::get($this->stub('addon_service.php'));
				$servicesStub = $this->write('service_name', Str::studly($service), $servicesStub);

				$servicesWriteData .= "\n\t\t" . $servicesStub . "\n";

				$serviceStub = File::get($this->stub('service.php'));
		...
		if(Arr::has($this->data, 'models') && ($models = Arr::get($this->data, 'models'))) {
			...
			File::makeDirectory($this->tempPath . 'Models');

			foreach ($models as $service) {

				if(!$service || $service == '') continue;

				$modelsStub = File::get($this->stub('addon_model.php'));
				$modelsStub = $this->write('model_name', Str::studly($service), $modelsStub);

				$modelsWriteData .= "\n" . $modelsStub . "\n";

				$modelStub = File::get($this->stub('model.php'));
		...
		if(Arr::has($this->data, 'consents') && ($consents = Arr::get($this->data, 'consents'))) {
			...
			foreach ($consents as $consent) {
				...
				$consentsStub = File::get($this->stub('addon_consent.php'));
		...
		if(Arr::has($this->data, 'cookies') && ($cookies = Arr::get($this->data, 'cookies'))) {
			...
			foreach ($cookies as $cookie) {
				...
				if(! isset($cookie['value']) || $cookie['value'] == '') continue;
			...
			foreach ($cookieData as $cookieType => $cookieValues) {
				$cookiesStub = File::get($this->stub('cookies.php'));
	...
	private function putFile($name, $contents, $path = null)
	{
		...
		File::put($this->tempPath . $path . $name, $contents);
	}
