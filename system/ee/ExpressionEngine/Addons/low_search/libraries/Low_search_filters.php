...
class Low_search_filters
{
    ...
    public function __construct()
    {
        ...
        if (! class_exists('Low_search_filter')) {
            require_once(PATH_ADDONS . 'low_search/filter.low_search.php');
        }
    ...
    private function _disabled($name)
    {
        ...
        if (in_array($name, ee()->low_search_settings->get('disabled_filters'))) {
            return true;
        }

        // Disabled in the tag (ie. disable="low_search:categories|low_search:keywords")
        if (REQ == 'PAGE' && ee()->low_search_params->in_param('low_search:' . $name, 'disable')) {
            return true;
        }
