...
*/
	...
	//			
			...
			if (!empty($headers_assoc['from'])) {
				$parsed_from = $this->extractEmailAddress($headers_assoc['from']);
				if ($parsed_from != '') {
					$this->sender_email = $parsed_from;
				}
			}
			
			if (!empty($headers_assoc['content-type'])) {
				$detected_charset = $this->extractCharsetFromContentType($headers_assoc['content-type']);
				if ($detected_charset != '') {
					$this->charset = $detected_charset;
				}
			}
			
			if (!empty($headers_assoc['received'])) {
				$ip = $this->extractSenderIpFromReceived($headers_assoc['received']);
				if ($ip != '') {
					$this->post_data['ip'] = $ip;
				}
			}
		 ...
		 log_message('error', 'boundery 1 ' . $this->post_data['subject']);
			...
			if ($this->moblog_array['moblog_channel_id'] != '0' && $this->moblog_array['moblog_file_archive'] == 'n') {
				...
				if ($this->moblog_array['moblog_field_id'] != 'none' or
					...
					preg_match("/[\<\{]field\:(.*?)[\}\>](.*?)[\<\{]\/_field\:(.*?)[\}\>]/", $this->body, $matches)
					) {
					$this->post_entry();
				} else {
					$this->emails_done++;
	...
	public function post_entry()
	{
		...
		$data = array(
			...
			'sticky' => (isset($this->post_data['sticky'])) ? $this->sticky : $this->sticky,
		...
		if (preg_match_all("/[\<\{]field\:(.*?)[\}\>](.*?)[\<\{]\/field\:(.*?)[\}\>]/", $this->body, $matches)) {
			eee()->db->select('channel_fields.field_id, channel_fields.field_name, channel_fields.field_label, channel_fields.field_fmt');
			ee()->db->from('channel_fields');
	...
	// ------------------------------
	// RFC 5322 header parsing helpers
	// ------------------------------
	private function splitHeadersBody($raw)
	{
		$parts = preg_split("/\r\n\r\n|\n\n|\r\r/", $raw, 2);
	...
	private function unfoldHeaders($headers_raw)
	{
	...
	private function decodeMimeHeaderValue($value)
	{
		$value = trim($value);
		if ($value === '') {
			return $value;
		...
		if (function_exists('iconv_mime_decode')) {
			$decoded = @iconv_mime_decode($value, 0, ee()->config->item('charset'));
			return is_string($decoded) ? trim($decoded) : $value;
		}
		return preg_replace('/\s+/', ' ', $value);
	}
	
	private function parseRfc5322Headers($headers_raw)
	{
		$headers_raw = $this->unfoldHeaders($headers_raw);
		$lines = preg_split("/\r\n|\n|\r/", $headers_raw);
		$headers = array();
		foreach ($lines as $line) {
			if (preg_match('/^([^:]+):\s*(.*)$/', $line, $m)) {
				$key = strtolower(trim($m[1]));
				$val = $this->decodeMimeHeaderValue($m[2]);
	...
	private function extractCharsetFromContentType($contentType)
	{
		if (is_array($contentType)) {
			$contentType = reset($contentType);
		}
		if (preg_match('/charset\s*=\s*"?([^;\"]+)"?/i', $contentType, $m)) {
			return trim(strtolower($m[1]));
		}
		return '';
	}
	
	private function extractEmailAddress($fromHeader)
	{
		$from = is_array($fromHeader) ? reset($fromHeader) : $fromHeader;
		if (preg_match('/<([^>]+)>/', $from, $m)) {
			return trim($m[1]);
		}
		// fallback: find the first token that looks like an email
	...
	private function extractSenderIpFromReceived($received)
	{
		$receivedLines = is_array($received) ? $received : array($received);
		foreach ($receivedLines as $line) {
			if (preg_match('/\[([0-9]{1,3}(?:\.[0-9]{1,3}){3})\]/', $line, $m)) {
				$ip = $m[1];
