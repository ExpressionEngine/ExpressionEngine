...
class Mailinglist_model extends CI_Model  {
	...
	function mailinglist_details($list_id = '', $perpage = 20, $rownum = 0)
	{

		$this->db->select('user_id, mailing_list.list_id, mailing_lists.list_title, email, ip_address');
		$this->db->from('mailing_list');
		$this->db->join('mailing_lists', 'mailing_lists.list_id = mailing_list.list_id', 'left');
		...
		if ($list_id != '')
		{
			$this->db->where('mailing_lists.list_id', $list_id);
		}
	...
	function mailinglist_count($list_id)
	{
		ee()->db->where('list_id', $list_id);
		ee()->db->from('mailing_list');
