    ...
    protected function includeCustomCSS($configHandle, $templateId, $cssPrefix)
    {
        ...
        if (! empty($templateId)) {
            ...
            if (! empty($templateModel)) {
                ee()->TMPL->fetch_and_parse($templateModel->TemplateGroup->group_name, $templateModel->template_name, false, $templateModel->site_id);
                if (! empty(ee()->TMPL->final_template)) {
                    ...
                    if ($cachedCss !== false) {
                        ...
                        if ($finfo['mtime'] >= ee()->TMPL->template_edit_date) {
                            $prefixedCss = $cachedCss;
                        }
                    }
                    //no cache file, or file too old
                    if (! isset($prefixedCss)) {
                        $cssTemplate = ee()->TMPL->parse_globals(ee()->TMPL->final_template);
