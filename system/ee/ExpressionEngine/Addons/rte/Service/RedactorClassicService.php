...
class RedactorClassicService extends AbstractRteService implements RteService {
    ...
    protected static $type = 'redactorClassic';
    ...
    protected function insertConfigJsById()
    {
        ...
        if (!empty($this->toolset)) {
            ...
            $config = $baseConfig;
            $configHandle = 'redactorclassicdefault0';
        }
    ...
    public static function defaultConfigSettings()
    {
        ...
        return array(
            'type' => static::$type,
            'toolbar' => static::defaultToolbars()['Redactor Classic Basic'],
            'height' => '200',
            'upload_dir' => 'all'
        );
    }
    ...
    public function toolbarInputHtml($config)
    {
            ...
            $fullToolbar = array_merge($selection, static::defaultToolbars()['Redactor Classic Full']['buttons']);//merge to get the right order
            $fullToolset = [];
            foreach ($fullToolbar as $i => $tool) {
                if (in_array($tool, static::defaultToolbars()['Redactor Classic Full']['buttons'])) {
                    $fullToolset[$tool] = lang($tool . '_rte');
                }
            }

            return ee('View')->make('rte:redactorclassic-toolbar')->render(
                [
                    'buttons' => $fullToolset,
                    'selection' => $selection,
                    'type' => 'buttons'
    ...
    public function pluginsInputHtml($config)
    {
            ...
            $fullToolbar = array_merge($selection, static::defaultToolbars()['Redactor Classic Full']['plugins']);
            $fullToolset = [];
            foreach ($fullToolbar as $i => $tool) {
                if ($tool == 'limiter') {
                    continue;//this one one is included based on whether setting is provided
                }
                if (in_array($tool, static::defaultToolbars()['Redactor Classic Full']['plugins'])) {
                    $fullToolset[$tool] = lang($tool . '_rte');
                }
            }
