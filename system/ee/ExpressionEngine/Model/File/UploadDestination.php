...
class UploadDestination extends StructureModel
{
    ...
    /**
     * Synchronize file data in database
     *
     * @param string $filePath
     * @return array $errors
     */
    public function syncFile($filePath, $missing_only_sizes = [], $errors = [])
    {
        ee()->load->library('filemanager');

        $fileInfo = $this->getFilesystem()->getWithMetadata($filePath);
        if (!isset($fileInfo['basename'])) {
            $fileInfo['basename'] = basename($fileInfo['path']);
        }
        $mime = ($fileInfo['type'] != 'dir') ? $this->getFilesystem()->getMimetype($filePath) : 'directory';
        ...
        // Clean filename
        $clean_filename = ee()->filemanager->clean_filename($fileInfo['path'], $this->getId(), array(
        ...
        if ($fileInfo['path'] != $clean_filename) {
            ...
            $clean_filename = ee()->filemanager->clean_filename($clean_filename, $this->getId(), array(
                'convert_spaces' => false,
                'ignore_dupes' => false
            ));
            // Rename the file
            if (! $this->getFilesystem()->rename($fileInfo['path'], $clean_filename)) {
                $errors[$fileInfo['path']] = lang('invalid_filename');
                return $errors;
            }

            $this->getFilesystem()->delete($fileInfo['path']);
            $fileInfo['basename'] = $this->getFilesystem()->basename($clean_filename);
        }
