...
class ChannelEntry extends ContentModel
{
    ...
    public function validate()
    {
        ...
        if (!empty($categoryGroupsSingleSelection)) {
            ...
            foreach ($categoryGroupsSingleSelection as $groupId) {
                $catsInGroup = explode('|', (string) $this->getProperty('cat_group_id_' . $groupId));
    ...
    public function validateUniqueUrlTitle($key, $value, $params, $rule)
    {
        ...
        if ($entry) {
            if (defined('REQ') && REQ == 'CP') {
                ...
                return sprintf(lang('url_title_not_unique'), $edit_link, htmlentities((string) $entry->title, ENT_QUOTES, 'UTF-8'));
            }
    ...
    public function onAfterSave()
    {
        ...
        if (ee('Request')->get('modal_form') == 'y' && ee('Request')->get('hide_closer') == 'y') {
            foreach ($this->Autosaves as $autosave) {
                ...
                foreach ($autosavedEntryData as $key => $val) {
                    if ($key == 'title' || strpos((string) $key, 'field_id_') === 0) {
    ...
    public function getDisplay(?LayoutInterface $layout = null)
    {
        ...
        $this->getCustomField('title')->setItem(
            'field_label',
            htmlentities((string) $this->Channel->title_field_label, ENT_QUOTES, 'UTF-8')
        );
            ...
            foreach (array_keys($providers) as $name) {
                try {
                    ...
                    if (file_exists($info->getPath() . '/tab.' . $name . '.php')
                        && in_array(ucfirst((string) $name), $installed_modules)) {
                        $modules[$name] = $info;
                    }
                } catch (\Exception $e) {
                    continue;
    ...
    protected function setDataOnCustomFields(array $data = array())
    {
        ...
        foreach ($data as $name => $value) {
            if (strpos((string) $name, 'field_ft_') === 0) {
