    ...
    <div class="list-item__secondary" style="left: 51px;">
      #<?=$category->data->cat_id?> <span class="faded">/</span> <span class="click-select-text"><?=$category->data->cat_url_title?></span>
		...
		<div class="list-item__checkbox">
			...
			<input id="cat_cb_<?=$category->data->cat_id?>" aria-label="<?=lang('select') . ' ' . $category->data->cat_name?>" type="checkbox" name="categories[]" value="<?=$category->data->cat_id?>" data-confirm="<?=lang('category') . ': <b>' . htmlentities($category->data->cat_name, ENT_QUOTES, 'UTF-8') . '</b>'?>">
		</div>
		<?php endif; ?>
	</div>
