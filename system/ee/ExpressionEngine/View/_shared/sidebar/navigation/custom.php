                    ...
                    <div class="ee-sidebar__items-<?=$class_suffix?>">
                        ...
                        <nav class="nav-custom">
                            ...
                            <?php if ($item->isSubmenu()) : ?>
                                <a class="js-dropdown-toggle ee-sidebar__item js-dropdown-hover" data-dropdown-use-root="true" data-dropdown-pos="right-start" href="#" title="<?= lang($item->title) ?>">
                                <?php if (!empty($item->icon)) : ?>
                                    <img src="<?=$item->icon?>" width="16" height="16" alt="<?=lang($item->title)?>" />
                                <?php else : ?>
                                    <span class="ee-sidebar__item-custom-icon"><?=substr(lang($item->title), 0, 1)?></span>
                                <?php endif; ?>
                                <span class="ee-sidebar__collapsed-hidden"><?= lang($item->title) ?></span></a>
                            ...
                            <?php else : ?>
                                <a class="ee-sidebar__item" href="<?= $item->url ?>" title="<?= lang($item->title) ?>">
                                    <?php if (!empty($item->icon)) : ?>
                                        <img src="<?=$item->icon?>" width="16" height="16" alt="<?=lang($item->title)?>" />
                                    <?php else : ?>
                                        <span class="ee-sidebar__item-custom-icon"><?=substr(lang($item->title), 0, 1)?></span>
                                    <?php endif; ?>
                                    <span class="ee-sidebar__collapsed-hidden"><?= lang($item->title) ?></span>
                                </a>
                            <?php endif; ?>
                            <?php endforeach; ?>
                        </nav>
