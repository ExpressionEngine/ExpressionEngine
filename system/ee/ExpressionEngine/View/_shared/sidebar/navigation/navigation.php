			...
			<div role="navigation" class="ee-sidebar <?php if (!isset($ee_cp_viewmode) || $ee_cp_viewmode != 'classic') : ?> hidden<?php endif; ?><?php if (isset($collapsed_nav) && $collapsed_nav == '1') : ?> ee-sidebar__collapsed<?php endif; ?>">
				...
				<a class="ee-sidebar__title js-dropdown-toggle" data-dropdown-use-root="true" data-dropdown-pos="bottom-center" title="<?=lang('go_to')?> <?=ee()->config->item('site_name')?>"><span class="ee-sidebar__title-name"><i class="fal fa-desktop fa-fw"></i><span class="ee-sidebar__collapsed-hidden"> <?=ee()->config->item('site_name')?></span></span><span class="ee-sidebar__title-down-arrow ee-sidebar__collapsed-hidden"><i class="fal fa-angle-down"></i></span></a>
				...
				<?php elseif (! ($site_name = ee()->config->item('site_name')) or empty($site_name)): ?>
					<a class="ee-sidebar__title ee-sidebar__title--needs-name" href="<?=ee('CP/URL', 'settings')?>" title="<?=lang('go_to')?> <?=lang('name_your_site')?>"><i class="fal fa-desktop fa-fw"></i> <span class="ee-sidebar__collapsed-hidden"><?=lang('name_your_site')?></span></a>
				<?php else: ?>
					<a class="ee-sidebar__title" href="<?=ee()->config->item('site_url')?>" rel="external" title="<?=lang('go_to')?> <?=ee()->config->item('site_name')?>"><span class="ee-sidebar__title-name"><i class="fal fa-desktop fa-fw"></i> <?=ee()->config->item('site_name')?></span></a>
				<?php endif ?>
