		...
		<?php foreach ($data as $heading => $rows):
			...
			<?php foreach ($rows as $row): ?>
				...
				<div<?php foreach ($row['attrs'] as $key => $value):?> <?=$key?>="<?=$value?>"<?php endforeach ?>>
					<?php if ($reorder): ?>
						<ul class="toolbar">
							<li class="reorder"><a href="#" title="<?=lang('reorder_row') . lang('button')?>"><span class="sr-only"><?=lang('reorder_row')?></span></a></li>
						</ul>
					<?php endif ?>
					...
					<?php endforeach ?>
					<ul class="toolbar">
						<li class="remove"><a href="#" rel="remove_row" title="<?=lang('remove_row') . lang('button')?>"><span class="sr-only"><?=lang('remove_row')?></span></a></li>
					</ul>
				</div>
			<?php endforeach ?>
		<?php endforeach ?>
