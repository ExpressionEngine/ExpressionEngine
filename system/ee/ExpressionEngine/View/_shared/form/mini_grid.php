		...
		<?php foreach ($data as $heading => $rows):
			...
			<?php foreach ($rows as $row): ?>
				...
				<div<?php foreach ($row['attrs'] as $key => $value):?> <?=$key?>="<?=$value?>"<?php endforeach ?>>
					<?php if ($reorder): ?>
						<ul class="toolbar">
							<li class="reorder"><a href="#" title="reorder row"><span class="sr-only">reorder row button</span></a></li>
						</ul>
					<?php endif ?>
					...
					<?php endforeach ?>
					<ul class="toolbar">
						<li class="remove"><a href="#" rel="remove_row" title="remove row"><span class="sr-only">remove row button</span></a></li>
					</ul>
				</div>
			<?php endforeach ?>
		<?php endforeach ?>
