<?php if (isset($buttons)): ?>
	...
	<?php foreach ($submits as $i => $button) :
		...
		$button['attrs'] = (isset($button['attrs'])) ? $button['attrs'] : '';
	?>
		<button class="btn button <?=$class?>" <?=$button['attrs']?> <?=$disabled?> name="<?=$button['name']?>" type="<?=$button['type']?>" value="<?=$button['value']?>" data-submit-text="<?=lang($button['text'])?>" data-work-text="<?=lang($button['working'])?>"><?=$button_html?><?=$button_text?></button>
