...
<div class="button-group <?=$class?> <?=$allow_multi?>">
    ...
    foreach ($choices as $key => $choice) :
        ...
        <label class="button button--default<?=($checked) ? ' active' : '' ?>">
            <input class="hidden" type="checkbox" name="<?=$field_name?>" value="<?=htmlentities((string) $key, ENT_QUOTES, 'UTF-8')?>"<?php if ($checked) :?> checked="checked"<?php endif; ?><?=isset($attrs) ? $attrs : ''?><?=$disabled?>>
