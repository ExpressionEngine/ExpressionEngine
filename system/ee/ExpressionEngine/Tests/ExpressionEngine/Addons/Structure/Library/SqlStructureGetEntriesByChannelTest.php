...
class SqlStructureGetEntriesByChannelTest extends TestCase
{
    /**
     ...
     * @return void
     */
    public function testGetEntriesByChannelReturnsFalseForNonNumericChannelIdWithoutQueryingDatabase(): void
    {
        $db = new class {
            public $queries = [];
            ...
            public function query($sql): void
            {
                ...
                throw new RuntimeException('get_entries_by_channel() should not query the database for non-numeric channel IDs.');
            }
        };
        ee()->setMock('db', $db);
        ...
        $this->assertFalse($sql->get_entries_by_channel('bad-channel'));
        $this->assertSame([], $db->queries);
    }

    /**
     * It returns ordered entry IDs for numeric-string channel IDs and scopes the query to the site.
     *
     * @return void
     */
    public function testGetEntriesByChannelReturnsEntryIdsForNumericStringChannelId(): void
    {
        $captured = (object) [
            'queries' => [],
        ...
        $this->assertSame([101, 205], $sql->get_entries_by_channel('4'));
        $this->assertSame(
            ['SELECT entry_id FROM exp_channel_titles WHERE channel_id = 4 AND site_id = 7'],
            $captured->queries
        );
     ...
     * @return void
     */
    public function testGetEntriesByChannelReturnsEmptyArrayWhenQueryFindsNoRows(): void
    {
        $captured = (object) [
            'queries' => [],
            'result_array_calls' => 0,
        ...
        $this->assertSame([], $sql->get_entries_by_channel(0));
        $this->assertSame(
            ['SELECT entry_id FROM exp_channel_titles WHERE channel_id = 0 AND site_id = 3'],
            $captured->queries
        );
        $this->assertSame(1, $captured->result_array_calls);
    }
     ...
     * @return void
     */
    public function testGetEntriesByChannelCoverageSubprocessCapturesCurrentBranchGap(): void
    {
        $outputFile = sys_get_temp_dir() . '/sql-structure-get-entries-by-channel-' . uniqid('', true) . '.json';
        $script = dirname(__DIR__, 4) . '/support/sql_structure_get_entries_by_channel_subprocess.php';
        ...
        $this->assertSame(
            [
                'SELECT entry_id FROM exp_channel_titles WHERE channel_id = 4 AND site_id = 7',
                'SELECT entry_id FROM exp_channel_titles WHERE channel_id = 0 AND site_id = 7',
            ],
            $result['queries']
        );
        $this->assertSame([101, 205], $result['numeric_string_result']);
