...
$function = new ReflectionFunction('pro_languages');
...
$config = new class {
    public $loadFileNames = [];
    public $scenario = 'populated';

    /**
     * Return controlled language config for cache coverage.
     *
     * @param string $name
     * @return array|null
     */
...
$replacementConfig = new class {
    public $loadFileNames = [];

    /**
     * Record unexpected cache-bypass calls.
     *
     * @param string $name
     * @return array
     */
    public function loadFile(string $name): array
    {
        $this->loadFileNames[] = $name;

        return [
            'zz_cache_bypass' => 'Cache Bypass',
        ];
    }
};
...
$firstCall = pro_languages();

ee()->setMock('config', $replacementConfig);

$secondCall = pro_languages();
...
$functionCoverage = $fileCoverage['functions']['pro_languages'] ?? ['branches' => [], 'paths' => []];
$coveredLines = [];
...
file_put_contents($outputFile, json_encode([
    ...
    'load_file_call_count' => count($config->loadFileNames),
    'load_file_names' => $config->loadFileNames,
    'replacement_config_load_file_names' => $replacementConfig->loadFileNames,
], JSON_PRETTY_PRINT));
