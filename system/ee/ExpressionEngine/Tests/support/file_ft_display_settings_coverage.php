...
require_once __DIR__ . '/../ExpressionEngine/Addons/file/FT/FileFtDisplaySettingsTest.php';
...
$method = new ReflectionMethod('File_ft', 'display_settings');
$xdebugAvailable = function_exists('xdebug_start_code_coverage') && function_exists('xdebug_get_code_coverage');

/**
 * Reset EE mocks and seed the shared File_ft display_settings() doubles.
 *
 * @param array<int|string, string> $dictionary
 * @param int $siteId
 * @return void
 */
function file_ft_display_settings_bootstrap(array $dictionary, $siteId)
{
    ee()->resetMocks();
    ...
    ee()->setMock('CP/URL', new FileFtCpUrlFactoryStub());

    $query = new FileFtDisplaySettingsUploadDestinationQueryStub($dictionary);
...
file_ft_display_settings_bootstrap([
    2 => 'Banners',
    9 => 'Documents',
], 7);
(new File_ft())->display_settings([]);
...
$crossSiteModel = new FileFtDisplaySettingsModelStub(
    new FileFtDisplaySettingsUploadDestinationQueryStub([
        4 => 'Downloads',
    ])
);
$crossSiteModel->setFirstResult(42, (object) [
    'name' => 'Shared Assets',
    'Site' => (object) ['site_label' => 'Secondary Site'],
]);
ee()->setMock('Model', $crossSiteModel);
ee()->setMock('CP/Alert', new FileFtDisplaySettingsAlertServiceStub());
(new File_ft())->display_settings([
...
$functionCoverage = $fileCoverage['functions']['File_ft->display_settings'] ?? ['branches' => [], 'paths' => []];
$coveredLines = [];
