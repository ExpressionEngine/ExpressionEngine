...
$method = new ReflectionMethod('File_ft', 'pre_process');
$xdebugAvailable = function_exists('xdebug_start_code_coverage') && function_exists('xdebug_get_code_coverage');

/**
 * Reset EE mocks and seed the shared File_ft pre_process doubles.
 *
 * @return FileFtFileFieldStub
 */
function file_ft_pre_process_bootstrap()
{
    ee()->resetMocks();
...
/**
 * Build a File_ft instance for pre_process() coverage runs.
 *
 * @return File_ft
 */
function file_ft_pre_process_make_fieldtype()
{
    return new File_ft();
}
...
$fileField = file_ft_pre_process_bootstrap();
$fileField->parseFieldReturn = ['file_id' => 12];
file_ft_pre_process_make_fieldtype()->pre_process('{filedir_1}banner.png');

$fileField = file_ft_pre_process_bootstrap();
$fileField->parseFieldReturn = null;
file_ft_pre_process_make_fieldtype()->pre_process('');
...
$functionCoverage = $fileCoverage['functions']['File_ft->pre_process'] ?? ['branches' => [], 'paths' => []];
$coveredLines = [];
