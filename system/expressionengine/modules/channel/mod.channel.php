...
class Channel {
	...
	function entries()
	{
		...
		{
			list($this->paginate, $this->field_pagination) = $this->EE->pagination->fetch_pagination_data();
		}
		...
		if ($this->EE->config->item('enable_sql_caching') == 'y')
		{
			...
			if (FALSE !== ($cache = $this->fetch_cache('pagination_count')))
			{
				if (FALSE !== ($this->fetch_cache('field_pagination')))
				{
					...
					{
						// TODO: Paginate and Field Pagination override?
						// TODO: Dynamic SQL is only true if we're in this loop
						$this->paginate = TRUE;
						$this->field_pagination = TRUE;
						$this->EE->pagination->create_pagination(trim($cache), $this->basepath, $this->per_page, $this->current_page, $this->display_by, $this->total_rows, $this->query_string, $this->dynamic_sql, $this->EE->db->query(trim($pg_query)));
					}
				}
				else
				{
					// TODO: Dynamic SQL is only true if we're in this loop
					$this->EE->pagination->create_pagination(trim($cache), $this->basepath, $this->per_page, $this->current_page, $this->display_by, $this->total_rows, $this->query_string, $this->dynamic_sql);
				}
			}
		}
		...
		if ($this->enable['pagination'] == TRUE)
		{
			$this->EE->pagination->add_pagination_data($this->current_page);
		}
	...
	function track_views()
	{
		...
		if ($this->field_pagination == TRUE AND $this->current_page > 0)
		{
			return;
		}
	...
	function build_sql_query($qstring = '')
	{
		...
		else
		{
			...
			else
			{
				...
				{
					$this->current_page = (isset($match[2])) ? $match[2] : $match[1];
		...
		// $sql_a is only the SELECT, nothing more
		$sql_a = "SELECT ";

		// $sql_b determines whether or not we need to deal with DISTINCT
		// For example, one post can belong to multiple categories, we don't 
		// want each instance of a post in category coming up each time
		...
		// TODO: remove, $sql_c doesn't even seem to be used
			...
			else
			{
				...
				if ($this->display_by == 'month')
				{
					...
					if ($query->num_rows() > 0)
					{
						...
						$cur = ($this->current_page == '') ? 0 : $this->current_page;
				...
				elseif ($this->display_by == 'week')
				{
					...
					if ($query->num_rows() > 0)
					{
						...
						*/
						...
						if ($this->EE->TMPL->fetch_param('show_current_week') === 'yes' && $this->current_page == '')
						{
							if ($this->EE->TMPL->fetch_param('start_day') === 'Monday')
							{
		...
		if ($this->paginate == TRUE) 
		{
			if ($this->field_pagination == FALSE)
			{
				...
				$this->EE->pagination->create_pagination($total, $this->basepath, $this->per_page, $this->current_page, $this->display_by, $this->total_rows, $this->query_string, $this->dynamic_sql);
			}
			else
			{
				$pager_sql = $sql_a.$sql_b.$sql;
				...
				$this->EE->pagination->create_pagination($total, $this->basepath, $this->per_page, $this->current_page, $this->display_by, $this->total_rows, $this->query_string, $this->dynamic_sql, $query);
		...
		if ($this->display_by == '')
		{
			...
			{
				$sql .= ($this->current_page == '') ? " LIMIT ".$offset.', '.$this->per_page : " LIMIT ".$this->current_page.', '.$this->per_page;
			}
			elseif ($entry_id == '' AND $qtitle == '')
			{
				$sql .= ($this->current_page == '') ? " LIMIT ".$this->limit : " LIMIT ".$this->current_page.', '.$this->limit;
			}
		}
		else
		{
	...
	function parse_channel_entries_new()
	{
		...
		foreach ($this->query->result_array() as $count => $row)
		{
			...
			$row['absolute_count']	= $this->current_page + $count + 1;
	...
	function parse_channel_entries()
	{
		...
		foreach ($query_result as $count => $row)
		{
			...
			$row['page_url']			= '';
			$row['total_results']		= $total_results;
			$row['absolute_count']		= $this->current_page + $row['count'];
