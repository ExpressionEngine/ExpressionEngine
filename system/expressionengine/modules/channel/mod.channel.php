...
class Channel {
	...
	*****************************************************************/
	...
	public function categories()
	{
		...
		else
		{
			...
			foreach ($this->cat_array as $key => $val)
			{
				...
				$cat_vars = array(
					'category_name'			=> htmlentities($val[3], ENT_QUOTES),
			...
			if ($query->num_rows() > 0)
			{
				...
				foreach($query->result_array() as $row)
				{
					...
					if ( ! isset($used[$row['cat_name']]))
					{
						...
						$cat_vars = array(
							'category_name'			=> htmlentities($row['cat_name'], ENT_QUOTES),
	...
	public function category_subtree($cdata = array())
	{
		...
		foreach($this->cat_array as $key => $val)
		{
			if ($parent_id == $val[0])
			{
				...
				$cat_vars = array(
					'category_name'			=> htmlentities($val[1], ENT_QUOTES),
	...
	public function category_heading()
	{
		...
		$cat_vars = array(
			'category_name'			=> htmlentities($query->row('cat_name'), ENT_QUOTES),
