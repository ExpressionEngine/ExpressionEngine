...
=====================================================
 File: mod.channel.php
-----------------------------------------------------
 Purpose: Channel class
=====================================================
*/
if ( ! defined('EXT'))
{
		...
		foreach ($this->query->result_array() as $count => $row)
		{
			...
			foreach ($this->EE->TMPL->var_pair as $key => $val)
			{
				...
				if (strncmp($key, 'date_heading', 12) == 0)
				{
					...
					if ($display == 'hourly')
					{
						$heading_date_hourly = date('YmdH', $this->EE->localize->set_localized_time($row['entry_date']));
					...
					elseif ($display == 'weekly')
					{
						...
						if (strtolower($this->EE->TMPL->fetch_param('start_day')) != 'monday' && date('w', $this->EE->localize->set_localized_time($row['entry_date'])) == 0)
						{
					...
					{
						$heading_date_monthly = date('Ym', $this->EE->localize->set_localized_time($row['entry_date']));
					...
					{
						$heading_date_yearly = date('Y', $this->EE->localize->set_localized_time($row['entry_date']));
