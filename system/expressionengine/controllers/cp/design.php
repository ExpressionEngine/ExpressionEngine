...
class Design extends Controller {
	...
	function template_access($template_id = '')
	{
		...
		if ($template_id == '')
		{
			if ( ! $template_id = $this->input->get_post('id'))
			{
				return false;
			}
		...
		{
			$message = $this->dsp->qdiv('success', $this->lang->line('preferences_updated'));
		}
		
		
		$query = $this->db->query("SELECT template_name, enable_http_auth, group_id, no_auth_bounce FROM exp_templates WHERE template_id = '".$this->db->escape_str($template_id)."'");
		...
		$this->dsp->title  = $this->lang->line('template_access');		
		$this->dsp->crumb  = $this->lang->line('template_access');	  
	
		$r  = $this->dsp->form_open(array('action' => 'C=templates'.AMP.'M=update_template_access', 'name' => 'templateManagement', 'id' => 'templateManagement'))
			 .$this->dsp->input_hidden('template_id', $template_id)
			 .$this->dsp->input_hidden('group_id', $group_id)
			 .$this->dsp->qdiv('tableHeading', $this->lang->line('template_access').NBS.'('.$template_name.')')
			 .$message;
		
		$r .= $this->dsp->qdiv('box', $this->dsp->qdiv('highlight_alt', $this->lang->line('group_restriction')));

		$r .= $this->dsp->table('tableBorder', '0', '', '100%').
			  $this->dsp->tr().
			  $this->dsp->td('tableHeading', '', '').
			  $this->lang->line('member_group').
			  $this->dsp->td_c().
			  $this->dsp->td('tableHeading', '', '').
			  $this->lang->line('can_view_template').
			  $this->dsp->td_c().
			  $this->dsp->tr_c();
		...
		$result = $this->db->query("SELECT member_group FROM exp_template_no_access WHERE template_id = '".$this->db->escape_str($template_id)."'");
		...
		$query = $this->db->query("SELECT group_id, group_title FROM exp_member_groups WHERE site_id = '".$this->db->escape_str($this->config->item('site_id'))."' AND group_id != '1' ORDER BY group_title");
		$access_e = array();
		...
		foreach ($query->result_array() as $row)
		{
			...
			$r2 .= $this->dsp->tr().
					$this->dsp->td($style, '40%').
					$row['group_title'].
					$this->dsp->td_c().
					$this->dsp->td($style, '60%');
				  
			$selected = ( ! isset($group[$row['group_id']])) ? 1 : '';
				
			$r2 .= $this->lang->line('yes').NBS.
					$this->dsp->input_radio('access_'.$row['group_id'], 'y', $selected).NBS.NBS.NBS;
				
			$selected = (isset($group[$row['group_id']])) ? 1 : '';
				
			$r2 .= $this->lang->line('no').NBS.
					$this->dsp->input_radio('access_'.$row['group_id'], 'n', $selected).NBS.NBS.NBS;
		...
		$r3 = $this->template_access_toggle($access_e).
			  $this->dsp->tr().
			  $this->dsp->td('tableCellOne', '40%').
			  $this->dsp->qdiv('defaultBold', $this->lang->line('select_all')).
			  $this->dsp->td_c().
			  $this->dsp->td('tableCellOne', '60%').
			  $this->lang->line('yes').NBS.
			  $this->dsp->input_radio('can_view', 'y', '', "onclick=\"toggle_access(this);\"").NBS.NBS.NBS.
			  $this->lang->line('no').NBS.
			  $this->dsp->input_radio('can_view', 'n', '', "onclick=\"toggle_access(this);\"").NBS.NBS.NBS.
			  $this->dsp->td_c().
			  $this->dsp->tr_c();
		...
		$r4 = $this->template_access_toggle($access_e).
			  ...
			  $this->dsp->td($style, '60%').
			  $this->lang->line('yes').NBS.
			  $this->dsp->input_radio('can_view2', 'y', '', "onclick=\"toggle_access(this);\"").NBS.NBS.NBS.
			  $this->lang->line('no').NBS.
			  $this->dsp->input_radio('can_view2', 'n', '', "onclick=\"toggle_access(this);\"").NBS.NBS.NBS.
			  $this->dsp->td_c().
			  $this->dsp->tr_c();
			
		$r .= $r3.$r2.$r4.$this->dsp->table_c(); 
		
		$r .= $this->dsp->div('box');
		$r .= $this->dsp->heading($this->lang->line('no_access_select_blurb'), 5);
		$r .= $this->dsp->qdiv('itemWrapper', $this->lang->line('no_access_instructions'));
		...
		$r .=  $this->dsp->div()
			  .$this->dsp->input_select_header('no_auth_bounce');
				
		foreach ($query->result_array() as $row)
		{
			$selected = ($row['template_id'] == $no_auth_bounce) ? 1 : '';
		
			$r .= $this->dsp->input_select_option($row['template_id'], $row['group_name'].'/'.$row['template_name'], $selected);
		}
		
		$r .= $this->dsp->input_select_footer().BR.BR; 
		$r .= $this->dsp->div('paddedTop');
		$r .= $this->dsp->heading($this->lang->line('enable_http_authentication'), 5);
		$r .= $this->dsp->qdiv('itemWrapper', $this->lang->line('enable_http_authentication_subtext'));
		$r .= $this->dsp->qdiv('itemWrapper', $this->dsp->qspan('alert', $this->lang->line('enable_http_authentication_note')));
		$r .= $this->dsp->qdiv('itemWrapper', $this->dsp->input_checkbox('enable_http_auth', 'y', ($enable_http_auth == 'y') ? 'y' : '')
										.NBS.NBS.$this->lang->line('enable_http_authentication')); 
		$r .= $this->dsp->div_c(); 
		$r .= $this->dsp->div_c(); 
		$r .= $this->dsp->div_c(); 
