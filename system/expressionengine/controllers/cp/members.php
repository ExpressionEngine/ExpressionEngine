...
//echo '<pre>'; print_r($G); exit;
	...
	/**
	 * // @confirm: this function not currently used, as "search" was rolled into "view/search"
	 * // need to revisit that, as this doesn't feel right.
	 * Do Member search
	 *
	 * @access	public
	 * @return	mixed
	...
	function do_member_search()
	{
		...
		$pageurl = BASE.AMP.'C=members'.AMP.'M=do_member_search';
		...
		else
		{
			...
			if (	$_POST['username'] 		== '' &&
				...
				{
					$this->functions->redirect(BASE.AMP.'C=members'.AMP.'M=member_search');
				}
			...
			foreach ($_POST as $key => $val)
			{
				if ($key == 'group_id')
				{
					if ($val != 'any')
					{
						$search_query[] = " g.group_id ='".$this->db->escape_str($_POST['group_id'])."'";
					}
				}
				...
				{	
					if ($val != '')
					{
						...
						{
							$search_query[] = $key." = '".$this->db->escape_str($val)."'";
						}
						else
						{
							$search_query[] = $key." LIKE '%".$this->db->escape_like_str($val)."%'";
						}
					}
				}
			}
		...
		$sql .= " WHERE m.group_id = g.group_id AND g.site_id = '".$this->db->escape_str($this->config->item('site_id'))."' AND ".$Q;
		...
		if ($total_count == 0)
		{
			return $this->member_search($this->dsp->qdiv('itemWrapper', $this->dsp->qdiv('alert', $this->lang->line('no_search_results'))));
		}
		...
		$r  = $this->dsp->qdiv('tableHeading', $this->lang->line('member_search_results'));	
		
		// "select all" checkbox

		$r .= $this->dsp->toggle();
		
		$this->dsp->right_crumb($this->lang->line('new_member_search'), BASE.AMP.'C=members'.AMP.'M=member_search');
		
		$this->dsp->body_props .= ' onload="magic_check()" ';
		
		$r .= $this->dsp->magic_checkboxes();
		
		// Declare the "delete" form
		
		$r .= $this->dsp->form_open(
								array(
										'action' => 'C=members'.AMP.'M=member_delete_confirm',
										'name'	=> 'target',
										'id'	=> 'target'
									)
							);
		...
		$r .= $this->dsp->table('tableBorder', '0', '', '100%').
			  $this->dsp->tr().
			  $this->dsp->table_qcell('tableHeadingAlt', $this->lang->line('username')).
			  $this->dsp->table_qcell('tableHeadingAlt', $this->lang->line('screen_name')).
			  $this->dsp->table_qcell('tableHeadingAlt', $this->lang->line('email')).
			  $this->dsp->table_qcell('tableHeadingAlt', $this->lang->line('ip_address')).
			  $this->dsp->table_qcell('tableHeadingAlt', $this->lang->line('join_date')).
			  $this->dsp->table_qcell('tableHeadingAlt', $this->lang->line('member_group')).
			  $this->dsp->table_qcell('tableHeadingAlt', $this->dsp->input_checkbox('toggleflag', '', '', "onclick=\"toggle(this);\"").NBS.$this->lang->line('delete')).
			  $this->dsp->tr_c();
		...
		foreach($query->result_array() as $row)
		{
			...
			$r .= $this->dsp->table_qcell($style, $screen);
			
			// Email
			
			$r .= $this->dsp->table_qcell($style,
									$this->dsp->mailto($row['email'], $row['email'])
									);				
			// IP Address
			...
			$r .= $this->dsp->td($style).
				  $this->localize->convert_timestamp('%Y', $row['join_date']).'-'.
				  $this->localize->convert_timestamp('%m', $row['join_date']).'-'.
				  $this->localize->convert_timestamp('%d', $row['join_date']).
				  $this->dsp->td_c();
			...
			$r .= $this->dsp->table_qcell($style, $this->dsp->input_checkbox('toggle[]', $row['member_id'], '', " id='delete_box_".$row['member_id']."'"));
		...
		$r .=  $this->dsp->div('crumblinks').
			  ...
			  $this->dsp->div_c().
			  $this->dsp->td_c().
			  $this->dsp->td('defaultRight');
		
		// Delete button
		
		$r .= $this->dsp->input_submit($this->lang->line('delete')).
			  $this->dsp->td_c().
			  $this->dsp->tr_c();
		...
		$this->dsp->title = $this->lang->line('member_search');
		$this->dsp->crumb = $this->dsp->anchor(BASE.AMP.'C=admin'.AMP.'area=members_and_groups', $this->lang->line('members_and_groups')).
					  $this->dsp->crumb_item($this->lang->line('member_search'));
		$this->dsp->body  = $r;
	}
