...
class Tools_communicate extends CP_Controller {
	...
	function _view_cache_data($state, $params)
	{
		...
		{
			$rows[] = array(
				'subject'		=> "<strong><a href='".BASE.AMP.'C=tools_communicate'.AMP.'M=view_email'.AMP.'id='.$email['cache_id']."'>".htmlentities($email['subject'], ENT_QUOTES)."</a></strong>",
	...
	function view_email()
	{
		...
		$vars['subject'] = htmlentities($query->row('subject'), ENT_QUOTES);
