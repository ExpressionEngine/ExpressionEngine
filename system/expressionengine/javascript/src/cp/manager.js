...
(function($) {
	...
	function createPrefsRow(currentrow, rowdata) {
		...
		headerrow.find('select').each(function() {
			...
			switch(this.name) {
				case 'template_type':		field.val(rowdata.type);
					break;
				case 'cache':				field.val(rowdata.cache);
					break;
				case 'allow_php':			field.val(rowdata.allow_php);
					break;
				case 'php_parse_location':	field.val(rowdata.php_parsing);
					break;
			}
	...
	$(document).ready(function() {
		...
		var txtarea = $('#template_data'), selection;
