...
//wtf dst_enabled?		
	...
	/**
	 * return array of errors generated by api method
	 * 
	 * @return array
	 */
	...
	/**
	 * reset errors to empty array, clearing previous errors
	 * 
	 * @return $this
	 */
	...
	/**
	 * add an array or a single error to the current errors array
	 * 
	...
	public function update_field(array $field_data)
	{
		...
		$this->EE->db->where('field_name', element('field_name', $field_data));
		...
		if ($field_type == 'file')
		{
			...
			$upload_dir_prefs = $this->EE->tools_model->get_upload_preferences();
		...
		if ($edit === TRUE)
		{
			...
			{
				$this->EE->db->update('channel_data', array('field_ft_'.$native_settings['field_id'] => $native_settings['field_fmt']));
			}
