...
//  function extractByIndex($p_index, $p_path="./", $p_remove_path="")
  ...
  function properties()
  {
	...
	if (@is_file($this->zipname))
	{
	  ...
	  if (($this->zip_fd = @fopen($this->zipname, FOPEN_READ)) == 0)
	  {
		// ----- Error log
...
// --------------------------------------------------------------------------------
  ...
  function privCreate($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
  {
	...
	if (($v_result = $this->privOpenFd(FOPEN_WRITE_CREATE_DESTRUCTIVE)) != 1)
	{
	  // ----- Return
	  //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, $v_result);
	  return $v_result;
  ...
  function privAdd($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
  {
	...
	// ----- Open the zip file
	//--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Open file in binary read mode");
	if (($v_result=$this->privOpenFd(FOPEN_READ)) != 1)
	{
	  // ----- Return
	  //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, $v_result);
	  return $v_result;
	...
	//--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Open file in binary read mode");
	if (($v_zip_temp_fd = @fopen($v_zip_temp_name, FOPEN_WRITE_CREATE_DESTRUCTIVE)) == 0)
	{
	  $this->privCloseFd();
  ...
  function privAddFileList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
  {
	...
	for ($j=0; ($j<count($p_list)) && ($v_result==1); $j++)
	{
	  ...
	  */
	  ...
	  if ((@is_file($p_filename)) OR ((@is_dir($p_filename)) && ! $p_remove_all_dir)) {
		// ----- Add the file
  ...
  function privAddFile($p_filename, &$p_header, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_options)
  {
	...
	else if ($p_remove_dir != "")
	{
	  ...
	  if ((substr($p_filename, 0, 2) == "./") OR (substr($p_remove_dir, 0, 2) == "./"))
	  {
	...
	*/
	...
	if (is_file($p_filename))
	{
	  ...
	  if (($v_file = @fopen($p_filename, FOPEN_READ)) == 0)
	  {
		// ----- Error log
	  ...
	  if (($v_file_compressed = @gzopen($p_filename.'.gz', FOPEN_WRITE_CREATE_DESTRUCTIVE)) == 0)
	  {
		// ----- Close the file
		fclose($v_file);
	  ...
	  if (($v_file_compressed = @fopen($p_filename.'.gz', FOPEN_READ)) == 0)
	  {
		// ----- Error log
  ...
  function privExtract(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
	...
	// ----- Check the path
	if (($p_path == "") OR ((substr($p_path, 0, 1) != "/") && (substr($p_path, 0, 3) != "../")))
	  $p_path = "./".$p_path;
  ...
  function privExtractByIndex(&$p_file_list, $p_index, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
	...
	// ----- Read each entry
	for ($i=0, $j_start=0, $v_nb_extracted=0; ($i<$v_central_dir['entries']) && ($j_start<count($v_index_list)); $i++)
	{
	  ...
	  $v_extract = false;
	  for ($j=$j_start; ($j<count($v_index_list)) && ( ! $v_extract); $j++)
	  {
  ...
  function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
	...
	if (!($p_entry['external']==0x41FF0010))
	{
	  ...
	  if ($p_entry['compressed_size'] == $p_entry['size'])
	  {
		...
		if (($v_dest_file = @fopen($p_entry['filename'], FOPEN_WRITE_CREATE_DESTRUCTIVE)) == 0)
		{
	  ...
	  else
	  {
		...
		if (($v_dest_file = @fopen($p_entry['filename'].'.gz', FOPEN_WRITE_CREATE_DESTRUCTIVE)) == 0)
		{
		  //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Error while opening '".$p_entry['filename']."' in write binary mode");
		...
		// ----- Uncompress
		if (($v_src_file = gzopen($p_entry['filename'].'.gz', FOPEN_READ)) == 0)
		{
  ...
  function privReadEndCentralDir(&$p_central_dir)
  {
	...
	// ----- Check the	//--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Comment length : ".$v_data['comment_size']);
  ...
  function privDeleteByIndex($p_index, &$p_result_list)
  {
	...
	for ($i=0, $j_start=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++)
	{
	  ...
	  // ----- Look if the index is in the list
	  for ($j=$j_start, $v_found = false; ($j<count($v_index_list)) && ( ! $v_found); $j++)
	  {
		//--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Look if index '$i' is in '".$v_index_list[$j]."'");
	...
	if (($v_result = $v_temp_zip->privOpenFd(FOPEN_WRITE_CREATE_DESTRUCTIVE)) != 1)
	{
	  $this->privCloseFd();
  ...
  function privMerge(&$p_archive_to_add)
  {
	...
	if (($v_result=$p_archive_to_add->privOpenFd(FOPEN_READ)) != 1)
	{
	  $this->privCloseFd();
  ...
  function privDuplicate($p_archive_filename)
  {
	...
	// ----- Open the temporary file in write mode
	//--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Open file in binary read mode");
	if (($v_zip_temp_fd = @fopen($p_archive_filename, FOPEN_READ)) == 0)
	{
	  $this->privCloseFd();
...
/* End of file Pclzip.php */
/* Location: ./system/expressionengine/libraries/Pclzip.php */