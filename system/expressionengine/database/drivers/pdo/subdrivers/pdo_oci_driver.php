...
class CI_DB_pdo_oci_driver extends CI_DB_pdo_driver {
	...
	protected function _delete($table)
	{
		if ($this->ar_limit)
		{
			$this->where('rownum <= ',$this->ar_limit, FALSE);
			$this->ar_limit = FALSE;
		}
	...
	{
		return 'SELECT * FROM (SELECT inner_query.*, rownum rnum FROM ('.$sql.') inner_query WHERE rownum < '.($this->ar_offset + $this->ar_limit + 1).')'
			.($this->ar_offset ? ' WHERE rnum >= '.($this->ar_offset + 1): '');
	}
