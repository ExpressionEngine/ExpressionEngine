...
class CI_DB_pdo_sqlsrv_driver extends CI_DB_pdo_driver {
	...
	protected function _delete($table)
	{
		if ($this->ar_limit)
		{
			return 'WITH ci_delete AS (SELECT TOP '.$this->ar_limit.' * FROM '.$table.$this->_compile_wh('ar_where').') DELETE FROM ci_delete';
		}
	...
	protected function _limit($sql)
	{
		...
		{
			return $sql.' OFFSET '.(int) $this->ar_offset.' ROWS FETCH NEXT '.$this->ar_limit.' ROWS ONLY';
		}
		...
		if ($this->ar_offset && ! empty($this->ar_orderby))
		{
			...
			else
			{
				...
				$field_regexp = ($this->_quoted_identifier)
					? '("[^\"]+")' : '(\[[^\]]+\])';
				for ($i = 0, $c = count($this->ar_select); $i < $c; $i++)
				{
					$select[] = preg_match('/(?:\s|\.)'.$field_regexp.'$/i', $this->ar_select[$i], $m)
						? $m[1] : $this->ar_select[$i];
				}
			...
			return 'SELECT '.$select." FROM (\n\n"
				...
				."\nWHERE ".$this->escape_identifiers('CI_rownum').' BETWEEN '.($this->ar_offset + 1).' AND '.$limit;
		}
