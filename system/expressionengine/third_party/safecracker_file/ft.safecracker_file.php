...
class Safecracker_file_ft extends File_ft
{
	...
	{
		//save the original field_name here
		$field_name = $this->field_name;
		
		//set a custom field name so it's not an array
		$this->field_name = 'var_'.$this->var_id;
		
		//use the channel field display_field method
		$output = $this->display_field($data);
		
		//restore the original field_name
		$this->field_name = $field_name;
		
		//if there was an error registered in save_var_field, display it
		if ($error = $this->EE->session->flashdata('safecracker_file_error'))
		{
			$output = '<div class="notice">'.$error.'</div>'.$output;
		}
	 ...
	 * 
	 * @param mixed $data the settings data
	 * 
	...
	{
		$field_name = 'var_'.$this->var_id;
		
		$output = '';
		
		//user has chosen a file from the existing files dropdown
		if ($this->EE->input->post($field_name.'_existing'))
		{
			$output = '{filedir_'.$this->settings['safecracker_upload_dir'].'}'.$this->EE->input->post($field_name.'_existing');
		}
		//user has not selected an existing file, or clicked the remove "X", so keep current value
		elseif ($this->EE->input->post($field_name.'_hidden') && ! $this->EE->input->post($field_name.'_remove'))
		{
			$output = '{filedir_'.$this->settings['safecracker_upload_dir'].'}'.$this->EE->input->post($field_name.'_hidden');
		}
		//user uploaded a new file
		elseif ( ! empty($_FILES[$field_name]['size']))
		{
			$this->EE->load->add_package_path(PATH_THIRD.'safecracker_file/');
			...
			//stash the error in flashdata to be displayed later in display_var_field
			if (array_key_exists('error', $data))
			{
				$this->EE->session->set_flashdata('safecracker_file_error', $data['error']);
			}
			//no errors, save the new file location
			else
			{
				$output = '{filedir_'.$this->settings['safecracker_upload_dir'].'}'.$data['file_name'];
			}
		}
