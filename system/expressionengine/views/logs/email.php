	...
	<section class="item-wrap log">
		...
		<?php else: ?>
			...
			<div class="item">
				...
				<h3><b><?=lang('date_logged')?>:</b> <?=$row['act_date']?><br><b><?=lang('username')?>:</b> <?=$row['username']?>, <b><abbr title="<?=lang('internet_protocol')?>"><?=lang('ip')?></abbr>:</b> <?=$row['ip_address']?></h3>
				<div class="message">
					<p><?=lang('sent_to')?> <b><?=$row['recipient_name']?></b>, <?=lang('subject')?>: <a href=""><?=$row['subject']?></a></p>
				</div>
			</div>
			...
			<div class="item">
				<ul class="toolbar">
					<li class="solo remove"><a href="" title="remove"></a></li>
				</ul>
				<div class="message">
					<p><?=$row['action']?></p>
				</div>
